...
import { getUserOrOrg } from "./Account.js";
...
export default class App {
    constructor(conf = {}) {
        ...
        if ((!this._config.url || this._config.url === APP_CONFIG_DEFAULTS.url) && this._config.token) {
            try {
                const decodedToken = jwt_decode(this._config.token);
                // Skip old hardcoded issuer
                if (decodedToken && decodedToken.iss && decodedToken.iss !== "triply.cc")
        ...
        if (this._config.httpProxy || this._config.httpsProxy) {
             ...
             */
            bootstrap();
            global.GLOBAL_AGENT.HTTP_PROXY = this._config.httpsProxy || this._config.httpProxy;
            global.GLOBAL_AGENT.HTTPS_PROXY = this._config.httpsProxy;
        }
        if (this._config.url.endsWith("/"))
            this._config.url = this._config.url.slice(0, -1);
    }
    getConfig() {
        return this._config;
    }
    ...
    async isCompatible(minimumVersion) {
        ...
        if (!apiInfo.version)
            throw getErr(`The TriplyDB API ${apiInfo.apiUrl} does not report its version.`);
    ...
    getPostProcessedApiUrl(urlFromApi) {
