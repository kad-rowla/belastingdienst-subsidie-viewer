...
const command = program
    ...
    .option("--overwrite", "Overwrite the asset if it already exists. By default, this script will add a new version if an asset already exists", false)
    .action(async () => {
    ...
    console.info(`> Finished uploading ${files.length} ${files.length > 2 ? "files" : "file"} to dataset ${options.dataset} in account ${(await account.getInfo()).accountName}`);
});
