...
describe("User", function () {
    ...
    it("get all users", async function () {
        ...
        expect(accounts.map((x) => x["_name"])).to.contain((await user.getInfo()).accountName);
    });
    ...
    it("ensure that we get an error when the user doesn't exist", async function () {
        await expect(app.getAccount("maocpwnuzrls")).to.eventually.be.rejectedWith("Failed to fetch account maocpwnuzrls. (It does not exist)");
    });
    // Error messages
    describe("Initialization", function () {
        it("Should throw error when user does not exist", async function () {
            return expect(App.get().getUser("Triply_")).to.eventually.rejectedWith("Failed to fetch user Triply_. (It does not exist)");
        });
        it("Should throw error when casting a user to an organization", async function () {
            const account = await App.get().getUser("wouter-1");
            return expect(account.asOrganization()).to.eventually.rejectedWith("Unable to cast user wouter-1 to an organization. A user cannot be cast to an organization.");
        });
    });
});
