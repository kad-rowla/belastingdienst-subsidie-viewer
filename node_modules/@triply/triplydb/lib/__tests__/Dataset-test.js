...
describe("Dataset", function () {
    ...
    describe("Import from store", function () {
        ...
        it("Import a boolean literal", async function () {
            const store = new Store();
            const term = DataFactory.namedNode("a:a");
            store.addQuad(term, term, DataFactory.literal("true", DataFactory.namedNode("http://www.w3.org/2001/XMLSchema#boolean")), term);
            await testDs.importFromStore(store);
            const dsInfo = await testDs.getInfo();
            expect(dsInfo.statements).to.equal(1);
        });
    ...
    describe("Assets", function () {
        ...
        it("add an asset", async function () {
            ...
            let assetCount = 0;
            for await (let asset of testDs.getAssets())
                asset && assetCount++;
    ...
    describe("With dataset that has graphs", function () {
        ...
        it("Should have graphs", async function () {
            let graphs = [];
            for await (let graph of testDs.getGraphs())
        ...
        it("download a graph", async function () {
            ...
            for await (let g of testDs.getGraphs()) {
                ...
                await expect(new Promise(async (resolve, reject) => zlib.gunzip(await fs.readFile(gzipped), (e, result) => e ? reject(e) : resolve(JSON.parse(result.toString("utf-8")))))).to.eventually.deep.equal(JSON.parse(await fs.readFile(gunzipped, "utf-8")));
        ...
        describe("Import from dataset", function () {
            ...
            it("import single graph", async function () {
                let testDsGraphs = [];
                for await (let graph of dsToImportFrom.getGraphs())
        ...
        describe("Service tests", function () {
            it("Should make, restart, and delete a service", async function () {
                ...
                const dsToImportFromGraphs = [];
                for await (const graph of dsToImportFrom.getGraphs())
                    dsToImportFromGraphs.push(graph);
                await testDs.importFromDataset(dsToImportFrom, {
                    graphMap: {
                ...
                let serviceList = [];
                for await (const s of testDs.getServices())
                    serviceList.push(s);
