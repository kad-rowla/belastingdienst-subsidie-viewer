        ...
        describe("Select-queries", async function () {
            ...
            it("Should cache page when needed", async function () {
                ...
                for (let file of await fs.readdir(tmpDir)) {
                    ...
                    const data = await new Promise(async (resolve, reject) => gunzip(await fs.readFile(file), (error, result) => {
                        if (error)
                            return reject(error);
                        resolve(JSON.parse(result.toString("utf-8")));
                    }));
                    data.responseText = "[]";
                    await fs.writeFile(file, await new Promise((resolve, reject) => gzip(JSON.stringify(data), (error, result) => {
                        if (error)
                            return reject(error);
                        resolve(result);
                    })));
