...
describe("Tokens", function () {
    ...
    describe("Rename account", function () {
        ...
        it("read-token", async function () {
            await expect(renameFunction(readAccount, accountName)).to.be.rejectedWith(TriplyDbJsError, /Failed to update account information of.*\(401: Unauthorized/);
        });
        it("write-token", async function () {
            await expect(renameFunction(writeAccount, accountName)).to.be.rejectedWith(TriplyDbJsError, /Failed to update account information of.*\(401: Unauthorized/);
        });
    ...
    describe("Create and delete dataset", function () {
        const createDeleteDsFunction = (account) => account
            ...
            .then((ds) => ds.delete());
        it("read-token", async function () {
            return expect(createDeleteDsFunction(readAccount)).to.eventually.rejectedWith(`Failed to add dataset ${CommonUnittestPrefix}-test-ds to account ${accountName}. (401: Unauthorized.)`);
        });
        it("write-token", async function () {
    ...
    it("Read own private dataset with read-only token", async function () {
        ...
        let datasets = [];
        for await (let ds of readAccount.getDatasets())
            ds && datasets.push(ds);
    ...
    describe("Errors should not expose token", function () {
        it("Directly in the error message", function () {
            ...
            expect(getErr("hallo").setMessage(`Something ${readToken} is not allowed`).message).to.equal("Something <token> is not allowed");
        });
        it("Should omit if the token is in the cause", function () {
            const innerError = new Error(`Something ${readToken} is not allowed`);
            expect(getErr(`FirstError`).setCause(innerError).message).to.equal("FirstError (Something <token> is not allowed)");
        });
    });
});
