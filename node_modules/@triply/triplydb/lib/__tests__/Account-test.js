...
describe("Account", function () {
    describe("Initialization", function () {
        it("Should throw error when account does not exist", async function () {
            ...
            return expect(App.get().getAccount("Triply_")).to.eventually.rejectedWith("Failed to fetch account Triply_. (It does not exist)");
        });
        it("Should throw error when default account without API token", async function () {
            const client = App.get();
            const account = await client.getAccount();
            return expect(account.getInfo()).to.eventually.rejectedWith(`Failed to fetch the current user, because no API token is configured. If you want to fetch the current user, you must create an API token on <https://triplydb.com/me/-/settings/tokens>.`);
        });
    });
});
