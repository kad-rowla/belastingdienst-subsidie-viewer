...
describe('Validator', () => {
  ...
  it('should validate instance data using a SPARQL Constraint', async () => {
    ...
    assert.isTrue(report.dataset.getObjects(components[0], sh.focusNode, null).pop()?.equals(ex.concat('InvalidCountry')))
    assert.isTrue(report.dataset.getObjects(components[0], sh.resultPath, null).pop()?.equals(ex.concat('germanLabel')))
    assert.isTrue(report.dataset.getObjects(components[0], sh.sourceShape, null).pop()?.equals(ex.concat('LanguageExampleShape')))
    assert.isTrue(report.dataset.getObjects(components[0], sh.resultMessage, null).pop()?.equals(message))
  })
...
export const writeQuads = async (quads: any[], opts?: any): Promise<string> =>
