...
select ?alias (group_concat(?item) as ?items) {
  {
    select ?alias ?key (sample(?item0) as ?item) {
      ...
      bind(concat(
        ...
        if (bound(?description),
           str(?description), 
           if (bound(?label), str(?label), ?key)
        ),

        if (
          bound(?label) && bound(?description),
          concat('\n   * ', str(?label), '\n   *\n   * ', str(?description),'\n   *'),
          if (
            bound(?description), 
            concat('\n   * ', str(?description),'\n   *'),
            if (
              ...
              concat('\n   * ', str(?key),'\n   *')
                                      ...
                                      '\n   * @type        ',str(?category),
        if(?parent='',     '', concat('\n   * @parent      ',str(?parent))),
        if(!bound(?domain),'', concat('\n   * @domain      ',str(?domain))),
        if(!bound(?range), '', concat('\n   * @range       ', str(?range))),
        '\n   */\n',
        '  \'',?key,'\': prefixer','(\'',?key,'\'),\n\n') as ?item0)
    }
    group by ?alias ?key
