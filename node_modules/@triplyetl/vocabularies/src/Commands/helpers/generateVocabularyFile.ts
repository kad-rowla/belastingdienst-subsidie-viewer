...
export default async function generateJsonFile (job: Job, vocab: Vocab): Promise<string> {
  ...
  if (!existsSync(sourceFile)) return await Promise.reject(new Error(`Could not find vocabulary definition file 'static/Vocabularies/Generated/${job.prefix}.ts'`))
  ...
  if (vocab.items !== undefined) {
    for (const [key, item] of Object.entries(vocab.items)) {
      ...
      if (item.description !== undefined) itemContent.push(`   * ${item.description.trim()}`, '   *')
      itemContent.push(`   * @type        ${item.category}`)
      if (item.parent !== undefined) itemContent.push(`   * @parent      ${item.parent}`)
      if (item.domain !== undefined) itemContent.push(`   * @domain      ${item.domain}`)
      if (item.range !== undefined) itemContent.push(`   * @range       ${item.range}`)
      itemContent.push('   */')
