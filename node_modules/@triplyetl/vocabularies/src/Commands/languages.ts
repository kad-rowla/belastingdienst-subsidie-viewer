...
export const languages: CommandModule = {
  command: 'languages',
  describe: 'generate language library',
  handler: async (argv) => {
    const languages = (JSON.parse(fs.readFileSync('node_modules/language-subtag-registry/data/json/registry.json', 'utf-8')) as Registry[]).filter((region) => region.Type === 'language' && region.Subtag.match(/^[a-z]+$/))

    const lines: string[] = ["import LanguageTag from '../Classes/LanguageTag.js'\n", 'const language = {']
    let i = 0
    ...
    const regions = (JSON.parse(fs.readFileSync('node_modules/language-subtag-registry/data/json/registry.json', 'utf-8')) as Registry[]).filter((region) => region.Type === 'region' && region.Subtag.match(/^[A-Z]+$/))
