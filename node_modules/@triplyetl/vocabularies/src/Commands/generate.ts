  ...
  handler: async (argv) => {
    ...
    if (argv.index === undefined || argv.index === false) {
      if (argv.prefixes === undefined && argv.all === undefined) {
        ...
        jobs = $jobs.filter((job) => (argv.prefixes as string[]).includes(job.prefix))
    ...
    const token = (argv.token !== undefined ? (argv.token as string) : undefined) ?? process.env.TOKEN
    if (token === undefined) {
...
`)
    ...
    for (const job of jobs) {
      ...
      if (!useCache) await loadFromTriplyDB(app, job)
