...
import { fromCsv, fromXlsx, fromXml, fromJson, fromTsv, fromShapeFile, fromOai, } from "../generic/middlewares/sources/index.js";
...
const assertUtilityKeys = (throwOn = ["$selector", "$sheetName", "$tagName"]) => (ctx, next) => {
...
describe("transform.record.enrich ", () => {
    ...
    it("should create new utility keys for XLSX file", () => {
        const etl = new Etl();
        etl.use(fromXlsx(Source.file("./src/__tests__/resources/xlsxFiles/addresses_and_cities.xlsx")), assertUtilityKeys(["$tagName", "$selector"]));
        etl.use((ctx, next) => {
    ...
    it("should create new utility keys for XML file", () => {
        ...
        etl.use(fromXml(Source.file(file), { selectors: $selector }), assertUtilityKeys(["$sheetName"]), async (ctx, next) => {
