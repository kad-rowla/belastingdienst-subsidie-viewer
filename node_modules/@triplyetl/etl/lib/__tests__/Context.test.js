    ...
    describe("Getters", function () {
        ...
        it("getLiteral 'opts' should override previous 'language' or 'datatype' values", async () => {
            ...
            app.use(async (ctx, _next) => {
                ...
                expect(removeSomethingProperty.something).to.equal("shouldn't remove this property");
            });
            await app.run();
        });
        it("getIri should try to coerce to an Iri", async function () {
            ...
            app.use(async (ctx, _next) => {
                for (let key in coercionToIri) {
                    const gottenNamedNode = ctx.getIri(key);
        ...
        it("getBlankNode should try to coerce to a BlankNode", async function () {
            ...
            app.use(async (ctx, _next) => {
                for (let key in coercionToBlankNode) {
                    const gottenBlankNode = ctx.getBnode(key);
        ...
        it("getArray should do a runtime type check/conversion on all the elements of the array before returning", async function () {
            ...
            app.use(async (ctx, _next) => {
                // Strings
                expect(ctx.getArray("str", "string").every((element) => {
                    return typeof element === "string";
                })).to.equal(true);
                expect(ctx.getArray(jpath("$.str"), "string").every((element) => {
                    return typeof element === "string";
                })).to.equal(true);
                // Numbers
                expect(ctx.getArray("num", "number").every((element) => {
                    return typeof element === "number";
                })).to.equal(true);
                expect(ctx.getArray(jpath("$.num"), "number").every((element) => {
                    return typeof element === "number";
                })).to.equal(true);
                // Booleans
                expect(ctx.getArray("bool", "boolean").every((element) => {
                    return typeof element === "boolean";
                })).to.equal(true);
                expect(ctx.getArray(jpath("$.bool"), "boolean").every((element) => {
                    return typeof element === "boolean";
                })).to.equal(true);
                // 1 Deep Nested Array
                expect(ctx.getArray("arr", { array: "string" }).every((element) => {
                ...
                expect(ctx.getArray(jpath("$.arr"), { array: "string" }).every((element) => {
                    if (Array.isArray(element)) {
                        return element.every((el) => typeof el === "string");
                    }
                    else
                ...
                // 2 Deep Nested Array
                expect(ctx.getArray("nestedArr", { array: { array: "string" } }).every((element) => {
                    if (Array.isArray(element)) {
                ...
                expect(ctx.getArray(jpath("$.nestedArr"), { array: { array: "string" } }).every((element) => {
                    if (Array.isArray(element)) {
                        return element.flatMap((el) => el).every((el) => typeof el === "string");
                    }
                    else
        ...
        it("should get parent and root", async () => {
            ...
            app.use(fromJson(data, { skipEnrich: true }), forEach("L1", async (ctx, next) => {
