 ...
 * standard}.
 */
export default function _geojsonToWkt(args) {
    return addMwCallSiteToError(add({
        key: args.key,
        value: (context) => {
            let content = args.content;
            if (typeof args.content === "string" || isJPath(content))
                content = context.getAny(content);
            else if (isStatic(args.content))
                content = JSON.parse(args.content.toString());
            return ((args.crs !== undefined ? geospatial.wktCrsPrefix(context.getIri(args.crs)) : "") +
                wkx.Geometry.parseGeoJSON(content).toWkt());
        },
    }), { sourceFuncName: "_geojsonToWkt" });
}
