...
describe("Transform.split ", () => {
    it("should work on single static string, with and/or without limit and/or separator", () => getEtlForTesting([
        ...
        async (ctx, next) => {
            assert.isTrue(checkvalueNoSeperatatorNoLimit.length === ctx.getArray("splittedNoSeperatatorNoLimit", "string").length);
            assert.deepEqual(checkvalueNoSeperatatorNoLimit, ctx.getArray("splittedNoSeperatatorNoLimit", "string"));
            assert.isTrue(checkvalueWithSeperatatorNoLimit.length === ctx.getArray("splittedWithSeperatatorNoLimit", "string").length);
            assert.deepEqual(checkvalueWithSeperatatorNoLimit, ctx.getArray("splittedWithSeperatatorNoLimit", "string"));
            assert.isTrue(checkvalueWithSeperatatorRegExpNoLimit.length ===
                ctx.getArray("splittedWithSeperatatorRegExpNoLimit", "string").length);
            assert.deepEqual(checkvalueWithSeperatatorRegExpNoLimit, ctx.getArray("splittedWithSeperatatorRegExpNoLimit", "string"));
            return next();
        },
    ]).run());
    it("should work on single key, with and/or without limit and/or separator", () => getEtlForTesting([
    ...
    it("should remove empty entities by default", () => getEtlForTesting([
    ...
    it("should remove empty entities if specified", () => getEtlForTesting([
        split({
    ...
    it("should keep empty entities if parameter is set", () => getEtlForTesting([
        split({
            content: str(phoneNumbers),
            separator: phoneSeparator,
