    ...
    it("should create Skolem Iris (https://git.triply.cc/triply/etl/-/issues/221)", () => {
        ...
        return getEtlForTesting([
            ...
            async (ctx, _) => {
                ...
                assert.isTrue(skolems[0].subject.value.startsWith(skolemPrefix), `Iri '${skolems[0].subject.value}' does not start with skolemPrefix '${skolemPrefix}'`);
                assert.isTrue(skolems[1].subject.value.startsWith(skolemPrefix), `Iri '${skolems[1].subject.value}' does not start with skolemPrefix '${skolemPrefix}'`);
            },
        ]).run();
    });
...
//# sourceMappingURL=nestedPairs.test.js.map