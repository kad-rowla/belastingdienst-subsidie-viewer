    ...
    it("should create a quad from static strings", () => {
        ...
        return getEtlForTesting([
            ...
            async (ctx, next) => {
                ...
                assert.equal(1, ctx.store.getQuads({ graph, subject: dataFactory.namedNode(subject) }).length, "Incorrect quads count for 1st graph with S");
                assert.equal(1, ctx.store.getQuads({
                    graph,
                    subject: dataFactory.namedNode(subject),
                    predicate: dataFactory.namedNode(predicate),
                }).length, "Incorrect quads count for 1st graph with S,P");
                assert.equal(1, ctx.store.getQuads({
                    graph,
                    subject: dataFactory.namedNode(subject),
                    predicate: dataFactory.namedNode(predicate),
                    object: dataFactory.literal(object.toString()),
                }).length, "Incorrect quads count for 1st graph with S,P,O");
                return next();
            },
        ]).run();
    });
