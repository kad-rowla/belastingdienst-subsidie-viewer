...
describe("writing.triples", () => {
    it("should create 5 triples in 2 triples", () => {
        return getEtlForTesting([
            ...
            triples(graph, [vocab.sdo.Brewery, vocab.rdf.type, vocab.sdo.FoodEstablishment], [vocab.sdo.Brewery, vocab.sdo.name, literal(str("Heineken Brewery"), vocab.languageTag("en"))], [vocab.sdo.Brewery, vocab.sdo.name, str("Heineken")]),
            triples(anotherGraph, [vocab.sdo.Brewery, vocab.sdo.name, "objectIri"], [vocab.sdo.Brewery, vocab.sdo.name, "naamPrimitive"]),
            async (ctx, next) => {
    ...
    it("should create a quad from strings stored in keys", () => {
        ...
        return getEtlForTesting([
            ...
            async (ctx, next) => {
                ...
                assert.equal(1, ctx.store.getQuads({ graph, subject: subjectNode, predicate: predicateNode, object: objectNode }).length);
                return next();
            },
        ]).run();
    });
...
//# sourceMappingURL=triples.test.js.map