...
describe("SparqlUpdate", () => {
    it("Should insert data", async () => {
        ...
        app.use(triple(app.prefix.ex.concat("Christopher_Columbus"), app.prefix.ex.concat("explored"), app.prefix.ex.concat("India")));
        app.use(update(`INSERT DATA { GRAPH <${app.defaultGraph.value}> { <${app.prefix.ex.concat("Christopher_Columbus").value}> <${app.prefix.ex.concat("explored").value}> <${app.prefix.ex.concat("Caribbean").value}> } }`));
        let quads = [];
        app.use(async (ctx) => {
    ...
    it("should fix issues #233", async () => {
        ...
        etl.use(update(`base <${etl.standardGraphs.default}>\ninsert data { <s> <p> <o>. }`), update(`base <${etl.standardGraphs.default}/>\ninsert { graph ?g { ?o ?p ?s. } }\nwhere { graph ?g { ?s ?p ?o. } }`), async (ctx, next) => {
