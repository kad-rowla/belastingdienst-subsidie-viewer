 ...
 */
function sortByIndicePosition(position) {
    return [
        (runtime) => runtime.mwIndicesPosition[position],
        (runtime) => runtime.mwIndicesPosition.length,
    ];
}
export default function writeLogsToFile(opts) {
    ...
    const sortedRuntimes = sortBy(runtimes, 
    /**
    ...
    function formatMilliseconds(milliseconds) {
        ...
        const formattedTime = microseconds[1]
            ? `${String(hours).padStart(2, "0")}:${String(minutes).padStart(2, "0")}:${String(seconds).padStart(2, "0")}.${String(ms).padStart(3, "0")}${String(microseconds[1].padStart(2, "0"))}`
            : `${String(hours).padStart(2, "0")}:${String(minutes).padStart(2, "0")}:${String(seconds).padStart(2, "0")}.${String(ms).padStart(3, "0")}`;
        return formattedTime;
    }
    function millisecondsToHoursMinsSeconds(milliseconds) {
    ...
    const rows = sortedRuntimes.map((info) => {
        ...
        if (totalNumOfRecords < numOfRecords)
    ...
    const otherTime = opts.totalRuntimeMs -
