...
function termGetterToString(value) {
    if (value === undefined)
        return "unset";
...
function indentMultipleLines(text) {
    if (!text)
        return "";
    return text
        .split("\n")
...
export default function writeToFile(error, info) {
    ...
    if (triplyDbJsInfo) {
        ...
        if (triplyDbJsInfo.statusCode)
            lines.push(`  Status code: ${triplyDbJsInfo.statusCode}`);
...
//# sourceMappingURL=errorLogFile.js.map