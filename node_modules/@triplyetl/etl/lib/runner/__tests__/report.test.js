...
describe("Terminal Reports", () => {
    it("Should run and update the correct message count", async () => {
        ...
        async (ctx, next) => {
            ...
            return next();
        }, concat({ content: ["firstName", "familyName"], key: "name", separator: " " }), pairs(iri(app.standardPrefixes.id, "$recordId"), [a, sdo.Person]), validate(Source.string(shape), { terminateOn: "Never" }), async (ctx, next) => {
