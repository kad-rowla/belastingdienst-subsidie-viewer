...
export function useProgress(app) {
    const [progress, setProgress] = useState();
    useEffect(() => {
        if (!app)
            return;
        function listener(newProgress) {
...
export function useMessageCounts(app, hasFatalError) {
    ...
    if (hasFatalError) {
        if (messageCounts["Error"] === undefined)
            messageCounts["Error"] = { count: 0, level: "Error", type: "Etl" };
...
export function useStats(app) {
    ...
    useEffect(() => {
        ...
        function listener(stats) {
            if (stats["#Statements"] && parseInt(stats["#Statements"].toString()) % 1000 === 0)
                setStats({ ...stats });
        }
...
export function useEtl(getEtl) {
    const [app, setApp] = useState();
    const [status, setStatus] = useState();
    const [finalReport, setFinalReport] = useState();
    const [untouchedMiddlewares, setUntouchedMiddlewares] = useState();
    const { offset, etlFile, limit, timeout, token, tokenApiUrl, dataDir, account, verbosity, debugTrace, logDir, logId, errorTrace, createErrorTrace, } = useCliContext();
    ...
    useEffect(() => {
        if (getEtl) {
            ...
            Promise.resolve(Etl.CliContext?.triplydb?.getInfo())
                ...
                try {
                    if (!app) {
                        throw new Error(`File '${etlFile}' does not return any value. Expected the default function to end with \`return app;\``);
                    }
                    if (!(app instanceof Etl)) {
                        throw new Error(`File '${etlFile}' does not return a Etl instance. Expected the default function to end with \`return app;\``);
                    }
                    if (app.status === "done") {
                        throw new Error(`File '${etlFile}' returns a Etl instance that already ran. Make sure this function does _not_ call \`app.run()\`. This is something this Etl runner already does for you.`);
                    }
                    setApp(app);
                    ...
                    if (diff === -1) {
                        app.warn(`Please upgrade your node version (currently ${process.version}) to ${packageJson.engines.node}.`);
                    }
    ...
    return { app, error, finalReport, untouchedMiddlewares, status };
}
