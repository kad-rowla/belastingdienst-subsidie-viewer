...
import { coerceToString } from "../../../utils/index.js";
import { jwtDecode } from "jwt-decode";
export default function rdf(accountOrDatasetName, datasetNameOrOpts, optionalOpts) {
    return {
        get: async (app) => {
            ...
            async function getDataset() {
                if (_datasetObj)
                    return Promise.resolve(_datasetObj);
                return triplyDb
                    ...
                    if (config.token) {
                        const decodedToken = jwtDecode(config.token);
                        config.token = "[hidden]";
            ...
            const graphObjects = {};
            async function getGraphObject(graphName) {
            ...
            async function getId() {
                ...
                if (graphs) {
                    id += (await Promise.all(graphs.map(async (graphName) => {
            ...
            async function fetchToCacheWhenNeeded() {
                ...
                if (graphs) {
                    for (const graphName of graphs) {
                        ...
                        const modifiedTime = info.uploadedAt || info.importedAt;
                        if (!modifiedTime)
                            throw new Error("Missing graph date metadata");
            ...
            const humanReadableFilename = filenamify(`${accountName}-${datasetName}-${opts?.graphs?.length || 1}graph.trig.gz`, {
                replacement: "-",
            });
            async function getStream() {
                ...
                return new _Multistream(cacheFiles.map((f) => fs.createReadStream(f)));
            }
            ...
            return [
                {
                    ...
                    getSize: async () => {
                        ...
                        const cacheFiles = await fetchToCacheWhenNeeded();
                        return sum(await Promise.all(cacheFiles.map(async (f) => {
                    ...
                    getLocalPath: async () => {
                        ...
                        if (cacheFiles.length === 1)
                            return cacheFiles[0];
                        const localPath = path.resolve(cacheDir, `${accountName}-${datasetName}-concatenated-${md5(cacheFiles.join("-"))}.trig.gz`);
