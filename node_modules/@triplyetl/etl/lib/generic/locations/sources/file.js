...
import { createReadStreamSafe, parsePath, streamToString } from "../../../utils/files.js";
import mime from "mime";
import { castArray } from "lodash-es";
import { createHash } from "crypto";
export default function file(oneOrMorefilenames) {
    const parsedFilenames = castArray(oneOrMorefilenames).map((f) => [f, parsePath(f)]);
    return {
        get: async (_app) => {
            return Promise.all(parsedFilenames.map(async (pair) => {
