...
import { coerceToIri } from "../../../utils/index.js";
import { Store, getFactory, getStreamParser } from "@triplydb/data-factory";
import crypto from "crypto";
import { castArray } from "lodash-es";
import { sh } from "@triplyetl/vocabularies";
function getDatasetUrl(apiConfig, datasetInfo) {
    return `${apiConfig.consoleUrl}/${datasetInfo.owner.accountName}/${datasetInfo.name}`;
}
const isUpdateDataSet = (val) => typeof val === "object" && Object.hasOwn(val, "name");
export default function rdf(accountNameOrDatasetName, optionsOrDatasetName, optionalOptions) {
    ...
    if (typeof accountNameOrDatasetName === "string" && typeof optionsOrDatasetName === "string") {
        ...
        datasetMetadata = { name: accountNameOrDatasetName };
        if (typeof optionsOrDatasetName === "object") {
            options = { ...options, ...optionsOrDatasetName };
        }
    }
    ...
    const url = (options.triplyDb === undefined || options.triplyDb instanceof TriplyDb
        ? options.triplyDb
        : TriplyDb.get(options.triplyDb))?.getConfig()?.url;
    ...
    return {
        ...
        init: async (app) => {
            ...
            const metadata = {};
            if (datasetMetadata.accessLevel)
                metadata.accessLevel = datasetMetadata.accessLevel;
            if (datasetMetadata.displayName)
                metadata.displayName = datasetMetadata.displayName;
            if (datasetMetadata.description)
                metadata.description = datasetMetadata.description;
            if (datasetMetadata.license)
                metadata.license = datasetMetadata.license;
            try {
                ...
                if (datasetnameIsObject) {
                    if (options.existingMetadata !== "keep") {
                        ...
                        if (options.existingMetadata === "merge") {
                            ...
                            if ((info.displayName ?? "") === "")
                                metadataUpdate.displayName = metadata.displayName;
                            if ((info.description ?? "") === "")
                                metadataUpdate.description = metadata.description;
                            if (info.license === undefined)
                                metadataUpdate.license = metadata.license;
                            if ((info.exampleResources ?? []).length === 0 &&
                        ...
                        if (Object.keys(metadata).length > 0)
                            await ds.update(metadataUpdate);
                    }
                    else {
        ...
        postProcess: async (app, filesWithMetaData) => {
            ...
            if (tdb.getConfig().url !== info.apiUrl)
                app.info(`  - using API URL: ${tdb.getConfig().url}`);
            ...
            try {
                ...
                if (graphsInDataset.length) {
                    ...
                    let _Multistream = Multistream;
                    const statements = new pumpify.obj([
                        new _Multistream(files.map((f) => {
            ...
            const synchronizeServices = typeof options.synchronizeServices === "boolean"
            ...
            await Promise.all(services.map(async (service) => {
                ...
                if (synchronizeServices && filter(info)) {
                    if (["removing", "stopping", "stopped"].includes(info.status)) {
                        app.info(`Service ${info.name} in dataset ${datasetUrl} will not be synchronized as it has the status '${info.status}'.`);
                        stats.skipped++;
                        return;
                    }
                    if (info.status === "error") {
                        throw new Error(`Service ${info.name} in dataset ${datasetUrl} cannot be synchronized as it has an error.`);
                    }
            ...
            if (services.length > 0) {
                if (options.synchronizeServices) {
                    if (stats.total === stats.updated) {
                        app.info(`Synchronized ${stats.total === 1 ? "the service" : `all ${stats.total} services`} in ${datasetUrl}`);
                    }
                    else {
...
async function registerSource(source, tmpDir) {
