...
export default function fromXlsx(oneOrMoreSources, _opts = {}) {
    const opts = merge({}, DEFAULTS, _opts);
    return addMwCallSiteToError(async function _fromXlsx(ctx, next) {
        ...
        const xlsFiles = await Promise.all(sources.map(async (s) => {
        ...
        const records = flatten(xlsFiles.map((xls) => {
            ...
            return flatten(sheetsToRead.map((sheetName) => {
                ...
                if (isArray(xlsxArray) && isArray(xlsxHeaders) && new Set(xlsxHeaders).size !== xlsxHeaders.length) {
                    if (opts.groupColumnsByName) {
                        if (xlsxHeaders.includes(null)) {
                            ...
                            e.setMessage(`Undefined header found in file, please name all headers in source file.\n Encountered headers: ${JSON.stringify(xlsxHeaders)}`);
                            throw e;
                        }
                        const xlsxRows = xlsxArray.slice(1);
                        const dict = {};
                        xlsxHeaders.map((h) => (dict[h] = []));
                        xlsxHeaders.map((h, i) => xlsxRows.map((row) => dict[h].push(row[i])));
                        for (const key of Object.keys(dict)) {
                            dict[key] = dict[key].filter((n) => n !== null);
        ...
        for (const record of records) {
            for (const rowNum in record) {
                if (!isNaN(parseInt(rowNum))) {
                    ...
                    const row = !opts.skipTrim ? trimRecordFromSource(record[rowNum]) : record[rowNum];
                    const enriched = !opts.skipEnrich
...
//# sourceMappingURL=fromXlsx.js.map