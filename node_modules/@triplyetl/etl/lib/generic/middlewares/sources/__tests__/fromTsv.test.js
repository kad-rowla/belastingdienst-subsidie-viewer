...
describe("Read tsv documents", function () {
    ...
    it("should trim keys and values by default, but not when requested", async () => {
        ...
        appWithTrim.use(fromtsv(Source.file(filePath), {
            groupColumnsByName: true,
            escape: "\\",
            quote: '"',
            skipEnrich: true,
        ...
        appWithoutTrim.use(fromtsv(Source.file(filePath), {
            groupColumnsByName: true,
            escape: "\\",
            quote: '"',
            skipTrim: true,
    ...
    it("Should fix issue https://issues.triply.cc/issues/5631", async () => {
        const app = new Etl();
        app.use(fromtsv(Source.file("./src/__tests__/resources/tsvFiles/issue-redmine-5631.tsv"), {
            escape: "\\",
            quote: '"',
            skipEnrich: true,
        }), async (ctx, next) => {
            ...
            for (const key in ctx.record) {
                if (key === "") {
                    ...
                    assert.equal(ctx.getString(key), ctx.record[key].trim());
                }
            }
            return next();
        });
    ...
    describe("Should throw on duplicate when 'groupColumnsByName' option not set", async () => {
        it("using a file", async () => {
            ...
            await expect(app.run()).to.eventually.be.rejectedWith(new DuplicateColumnsError("./src/__tests__/resources/tsvFiles/duplicate-header-example.tsv").message);
        });
...
//# sourceMappingURL=fromTsv.test.js.map