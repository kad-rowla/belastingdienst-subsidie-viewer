...
async function recordsFromJson(files, opts) {
...
describe("fromJson", function () {
    ...
    it("should preprocess json object when needed", async function () {
        expect(await recordsFromJson(["./src/__tests__/resources/jsonFiles/objectWithArrayInside.json"], {
            toRecords: (json) => json.someArray,
    ...
    describe("Using Selectors with 'file' inputs", () => {
        it("should get records from a 'string' selector", async function () {
            const selectors = "a";
            expect(await recordsFromJson("./src/__tests__/resources/jsonFiles/abcdeNested.json", { selectors })).to.deep.equal([{ b: 2, $tagName: "a", $selector: "a" }]);
        });
        it("should get records from an 'array' of selectors", async function () {
            const selectors = ["b", "c.b"];
            expect(await recordsFromJson("./src/__tests__/resources/jsonFiles/abcdeNested.json", { selectors })).to.deep.equal([
        ...
        it("should get records from a mix of selectors", async function () {
            const selectors = "c.b";
            expect(await recordsFromJson("./src/__tests__/resources/jsonFiles/abcdeNested.json", { selectors })).to.deep.equal([{ a: 1, $tagName: "b", $selector: "c.b" }]);
        });
        ...
        it("should throw error when 2 selector paths conflict", async function () {
            ...
            await expect(recordsFromJson("./src/__tests__/resources/jsonFiles/abcdeNested.json", { selectors })).to.eventually.rejectedWith(/A pattern starts with another pattern/);
            // What if the selectors are the exact same
            await expect(recordsFromJson("./src/__tests__/resources/jsonFiles/abcdeNested.json", { selectors: ["c.b", "c.b"] })).to.eventually.rejectedWith(/A pattern starts with another pattern/);
        });
        it("should return empty array if there is no object found at key", async function () {
            const selectors = "e";
            await expect(recordsFromJson("./src/__tests__/resources/jsonFiles/abcdeNested.json", { selectors })).to.eventually.rejectedWith(/Only plain objects can be returned as records/);
        });
    });
    describe("Using selectors with 'data' inputs", () => {
        ...
        it("should throw error when 2 selector paths conflict", async () => {
            ...
            await expect(recordsFromJson(data, { selectors })).to.eventually.rejectedWith(/A pattern starts with another pattern/);
        });
        it("should throw if there is no object found at key", async () => {
            const selectors = "e";
            await expect(recordsFromJson(data, { selectors })).to.eventually.rejectedWith(/Only plain objects can be returned as records/);
        });
    });
    describe("Using saved queries", () => {
        ...
        it("should read ASK results from https://triplydb.com/academy/-/queries/ask-1", async () => {
            ...
            app.use((ctx, next) => {
                c++;
                assert.deepEqual(ctx.record, { boolean: true });
                return next();
            });
            await app.run();
...
//# sourceMappingURL=fromJson.test.js.map