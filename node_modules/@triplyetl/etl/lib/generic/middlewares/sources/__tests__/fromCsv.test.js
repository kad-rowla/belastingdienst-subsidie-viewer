...
import { Etl, Source, fromCsv } from "../../../index.js";
import { str } from "../../../../ratt/index.js";
export function getCountingMw() {
    const count = { count: 0 };
    return [
        count,
...
describe("Read csv documents", function () {
    ...
    it("should trim keys and values by default, but not when requested", async () => {
        ...
        appWithTrim.use(fromCsv(Source.file(filePath), {
        ...
        appWithoutTrim.use(fromCsv(Source.file(filePath), {
            groupColumnsByName: true,
            skipTrim: true,
            skipEnrich: true,
        }), async (ctx, next) => {
    ...
    describe("Should throw on duplicate when 'groupColumnsByName' option not set", async () => {
        it("using a file", async () => {
            ...
            await expect(app.run()).to.eventually.be.rejectedWith(new DuplicateColumnsError("./src/__tests__/resources/csvFiles/duplicate-header-example.csv").message);
        });
    ...
    it.skip("Should read duplicate column names containing additional spaces with 'groupColumnsByName' set", async () => {
        const app = new Etl({ defaultGraph: "https://default.com/" });
        app.use(fromCsv(Source.file("./src/__tests__/resources/csvFiles/duplicate-header-with-space.csv"), {
            groupColumnsByName: true,
        }), async (ctx, next) => {
...
//# sourceMappingURL=fromCsv.test.js.map