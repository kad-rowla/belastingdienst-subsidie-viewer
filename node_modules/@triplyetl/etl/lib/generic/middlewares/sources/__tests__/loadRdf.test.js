...
import { Etl, Source, compareGraphs } from "../../../index.js";
describe("loadRdf", function () {
    ...
    it("should load RDFa from HTML", function () {
        const app = new Etl();
        app.use(loadRdf(Source.file("./src/__tests__/resources/rdfFiles/john-doe.html"), {
    ...
    it("should work without content type for simple strings (see issue #229)", async () => {
        ...
        etl.use(loadRdf(Source.string(`<${graph.value}> { <${subject.value}> <${predicate.value}> <${object.value}> }`)), loadRdf(Source.string(`<${subject.value}> <${predicate.value}> <${object.value}> .`)), loadRdf(Source.string(`prefix ex: <http://ex.com/>\nex:s2 ex:p ex:o .`)), async (ctx, _) => {
    ...
    it("should apply a stylesheet export file (XSL)", async () => {
        ...
        etl.use(loadRdf(Source.file(xml), {
...
const assertJohnDoe = async (ctx, next) => {
...
//# sourceMappingURL=loadRdf.test.js.map