...
describe("Read shp documents", function () {
    ...
    it("Report problematic document", async function () {
        const app = new Etl({ defaultGraph: "https://default.com/" });
        app.use(fromShapeFile(Source.file("./src/__tests__/resources/shapeFiles/places-with-error.shp"), {
            skipEnrich: true,
        }));
    ...
    it("should not trim keys and values when requested", () => {
        const app = new Etl();
        app.use(fromShapeFile(Source.file("./src/__tests__/resources/shapeFiles/noot.shp"), { skipTrim: true }), async (ctx, next) => {
            assert.doesNotThrow(() => ctx.getAny("properties"));
...
//# sourceMappingURL=fromShapeFile.test.js.map