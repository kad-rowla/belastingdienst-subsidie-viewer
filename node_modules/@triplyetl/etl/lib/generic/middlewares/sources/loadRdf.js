...
import { addMwCallSiteToError, coerceToIri, md5OfArray } from "../../../utils/index.js";
...
const positions = ["graph", "subject", "predicate", "object"];
function needsToSanitize(quad) {
...
export async function getQuads(ctx, oneOrMoreSources, opts) {
...
const sourceToQuads = async (ctx, source, defaultGraph, opts) => {
    return new Promise(async (resolve, reject) => {
        ...
        if (contentType === "text/plain") {
            ...
            if (!contentType || !(await rdfParser.getContentTypes()).includes(contentType)) {
                return reject(new Error(`The contentType of your source ('${contentType}') is not recognized, if you are sure your RDF is valid, specify the format using the 'contentType' option.`));
            }
        }
        ...
        getParser(textStream, contentType, defaultGraph, useTriplyDbParser)
            .on("data", (quad) => {
            if (shouldUseTriplyParser(contentType))
                quadsOfSource.push(quad);
            else
                quadsOfSource.push(dataFactory().fromQuad(quad));
        })
            .on("error", (e) => {
            if (e instanceof Error)
                reject(new Error(`Failed to parse RDF ${source.name}: ${e.message}`));
            else
                throw e;
        })
            .on("end", async () => {
            ...
            for (let quad of quadsOfSource) {
                if (needsToSanitize(quad)) {
                    ...
                    function sanitizeTerm(term, graphName) {
                    ...
                    quads.push(dataFactory().quad(sanitizeTerm(quad.subject, quad.graph.value), sanitizeTerm(quad.predicate, quad.graph.value), sanitizeTerm(quad.object, quad.graph.value), sanitizeTerm(quad.graph, quad.graph.value)));
                }
                else {
                    quads.push(quad);
                }
...
function shouldUseTriplyParser(contentType) {
    return !!contentTypeMap[contentType];
}
function getParser(readStream, contentType, defaultGraph, useTriplyDbParser = false) {
    if (useTriplyDbParser || shouldUseTriplyParser(contentType)) {
        const triplyParserOptions = {
            baseIri: defaultGraph?.value,
            format: contentTypeMap[contentType],
        };
...
export default function loadRdf(oneOrMoreSources, opts) {
    return addMwCallSiteToError(async function _loadRdf(ctx, next) {
        ...
        const quads = await getQuads(ctx, oneOrMoreSources, opts);
        ctx.store.addQuads(quads);
        return next();
    }, { sourceFuncName: "_loadRdf" });
}
//# sourceMappingURL=loadRdf.js.map