 ...
 * - `POSTGRES_PASSWORD`
 */
export default function fromPostgres(query, opts = {}) {
    return addMwCallSiteToError(async function _fromPostgres(ctx, next) {
        assertOneExtractorPerETL(ctx.app, "fromPostgres");
        const options = { ...opts }; // make sure we don't overwrite the input
        const keys = ["host", "database", "username", "password"];
        for (const key of keys) {
            if (options[key] === undefined)
                options[key] = process.env["POSTGRES_" + key.toUpperCase()] || undefined;
        }
        ...
        if (options.showParsedOptions)
            console.info(sql.options);
        try {
...
//# sourceMappingURL=fromPostgres.js.map