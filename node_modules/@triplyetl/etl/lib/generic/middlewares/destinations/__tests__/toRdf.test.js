import { Etl, fromJson, Destination } from "../../../index.js";
...
async function compareFiles(lhs, rhs) {
    expect((await fs.readFile(lhs, "utf-8")).replace(/(?:\r\n|\r|\n)/g, "")).to.equal((await fs.readFile(rhs, "utf-8")).replace(/(?:\r\n|\r|\n)/g, ""));
}
async function compareFileWithString(file, rhs) {
...
describe("Write RDF", function () {
    ...
    it("Write with dynamic destination", async function () {
        const app = new Etl({
            defaultGraph: "",
            destinations: { d: (ctx) => Destination.file(path.resolve(tmpDataDir, `${ctx.recordId % 2}.nq`)) },
        });
        app.use(fromJson([
        ...
        await app.run();
        await compareFileWithString(path.resolve(tmpDataDir, "0.nq"), "<a2:a2> <b2:b2> <c2:c2> <d2:d2> .\n<a4:a4> <b4:b4> <c4:c4> <d4:d4> .");
        await compareFileWithString(path.resolve(tmpDataDir, "1.nq"), "<a1:a1> <b1:b1> <c1:c1> <d1:d1> .\n<a3:a3> <b3:b3> <c3:c3> <d3:d3> .");
    });
...
//# sourceMappingURL=toRdf.test.js.map