...
import { whenNot } from "../../index.js";
import { addMwCallSiteToError, randomHash } from "../../../utils/index.js";
import { getErr } from "../../../utils/Error.js";
import when from "./when.js";
import { unset } from "lodash-es";
const isElse = (IfThenElse) => Object.prototype.hasOwnProperty.call(IfThenElse, "else") &&
 ...
 * ```
 */
export default function ifElse(first, ...rest) {
    ...
    const setSentinelMw = (key) => {
        return addMwCallSiteToError((ctx, next) => {
            ctx.record[key] = true;
            return next();
    ...
    middlewares.push(when(sentinelKey, async (ctx, next) => {
        unset(ctx.record, sentinelKey);
        return next();
    }));
    return [
        addMwCallSiteToError((_ctx, next) => {
            return next();
        }, {
