 ...
 * ```
 */
export default function (when, ...middlewaresArray) {
    const whenFn = typeof when === "function" ? (ctx) => !when(ctx) : (ctx) => !ctx.hasKey(when) || ctx.isEmpty(when);
    whenFn.sourceFuncName = "_whenNot";
    whenFn.callsite = getCurrentLine({ frames: 2 });
    return whenMw(whenFn, middlewaresArray);
}
//# sourceMappingURL=whenNot.js.map