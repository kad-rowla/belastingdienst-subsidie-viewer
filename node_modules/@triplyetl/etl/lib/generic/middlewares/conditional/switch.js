 ...
 */
export function __switch(key, ...cases) {
    const IfThens = [];
    const Elses = [];
    ...
    cases.forEach((IfThenElse) => {
        if (isCaseWhen(IfThenElse)) {
            IfThens.push({
                if: (ctx) => ctx.getAny(key) === IfThenElse[0],
                then: IfThenElse[1],
            });
        }
 ...
 * )
 */
export default function _switch(key, ...cases) {
    return [
