...
describe("conditional.switch ", () => {
    it("should fail with empty arguments", () => assert.throw(() => getEtlForTesting(_switch("foo", [])), "`switch` middleware reported: no Cases provided."));
    it("should fail with no CaseWhen statements, just 1 Else", () => assert.throw(() => getEtlForTesting(_switch("foo", addValue({ key: "test", content: true }))), "`switch` middleware reported: no Cases provided, only a Default."));
    it("should fail with no CaseWhen statements, multiple Else", () => assert.throw(() => getEtlForTesting(_switch("foo", addValue({ key: "test1", content: true }), addValue({ key: "test2", content: true }))), "Only one default value allowed in a switch statement."));
    ...
    it("should work with 3 CaseWhen statements, stopping at the second CaseWhen, no Else statements", () => {
        return getEtlForTestingNoExtractor([
            ...
            _switch("foo", ["foo", addValue({ key: "testFoo", content: true })], ["bar", addValue({ key: "testBar", content: true })], ["aap", addValue({ key: "testAap", content: true })]),
            async (ctx, next) => {
    ...
    it("should work with 3 CaseWhen statements and Else statements, no CaseWhen matches", () => {
        return getEtlForTestingNoExtractor([
            fromJson({ foo: "bar" }, { skipEnrich: true }),
            _switch("foo", ["aap", addValue({ key: "aap", content: true })], ["noot", addValue({ key: "noot", content: true })], ["mies", addValue({ key: "mies", content: true })], addValue({ key: "wim", content: true })),
            async (ctx, next) => {
    ...
    it("should work with nested forEach middleware (#262)", async function () {
        const etl = new Etl();
        etl.use(fromJson([{ aap: [{}] }]), addValue({ content: "noot", key: "_test1" }), _switch("_test1", [
            "noot",
            forEach("aap", addSkolemIri({
                key: "_test2",
            })),
        ]), async (ctx, next) => {
