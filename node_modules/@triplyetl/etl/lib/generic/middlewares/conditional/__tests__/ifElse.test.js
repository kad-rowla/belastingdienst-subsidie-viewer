...
describe("conditional.ifElse ", () => {
    it("should fail on runtime with an If statements, multiple Else statements", () => {
        return expect(getEtlForTesting(ifElse({ if: "foo", then: addValue({ key: "test1", content: true }) }, { else: addValue({ key: "test2", content: true }) }, { else: addValue({ key: "test3", content: true }) })).run()).to.eventually.be.rejectedWith(ifElseErrorMessages.multipleElse);
    });
    ...
    it("should work with 3 If statements, stopping at the second If, no Else statements", () => {
        return getEtlForTesting([
            ifElse({ if: "bar", then: addValue({ content: true, key: "testBar" }) }, { if: "foo", then: addValue({ content: true, key: "testFoo" }) }, { if: "aap", then: addValue({ content: true, key: "testAap" }) }),
            async (ctx, next) => {
    ...
    it("should work when the ifElse is used more than once", () => {
        return getEtlForTesting([
            ...
            ifElse({ if: "foo", then: addValue({ key: "foo3", content: true }) }, { else: addValue({ key: "no-foo3", content: true }) }),
            ifElse({ if: "bar", then: addValue({ key: "no-foo4", content: true }) }, { else: addValue({ key: "foo4", content: true }) }),
            ifElse({ if: (_) => true, then: addValue({ key: "foo5", content: true }) }, { else: addValue({ key: "no-foo5", content: true }) }),
            ifElse({ if: (_) => false, then: addValue({ key: "no-foo6", content: true }) }, { else: addValue({ key: "foo6", content: true }) }),
            async (ctx, next) => {
                for (let i = 1; i <= 6; i++) {
    ...
    it("should work with 3 If statements and Else statements, Ifs do not fulfill", () => {
        return getEtlForTestingNoExtractor([
            ...
            ifElse({ if: "bar", then: addValue({ key: "not-foo1", content: true }) }, {
                if: (ctx) => ctx.getString("foo") === "Joe Doe",
                then: addValue({ key: "not-foo2", content: true }),
            }, { if: "aap", then: addValue({ key: "not-foo3", content: true }) }, { else: addValue({ key: "foo1", content: true }) }),
            async (ctx, next) => {
    ...
    it("should fail if Else statements is not the final argument", () => {
        return expect(getEtlForTesting(ifElse({ if: "foo", then: addValue({ key: "test1", content: true }) }, { else: addValue({ key: "test2", content: true }) }, { if: "foo", then: addValue({ key: "test3", content: true }) })).run()).to.eventually.be.rejectedWith(ifElseErrorMessages.wrongElsePosition);
    });
    it("should work with nested forEach middleware (#262)", async function () {
        const etl = new Etl();
        etl.use(fromJson([{ aap: [{}] }]), addValue({ content: "noot", key: "_test1" }), ifElse({
            if: "_test1",
            then: forEach("aap", addSkolemIri({
                key: "_test2",
            })),
        }, {
            else: forEach("aap", addSkolemIri({
                key: "_test2",
            })),
        }), async (ctx, next) => {
