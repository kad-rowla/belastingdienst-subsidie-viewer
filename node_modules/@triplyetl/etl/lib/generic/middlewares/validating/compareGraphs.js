...
import { getQuads } from "../sources/loadRdf.js";
...
export default (oneOrMoreSources, opts) => {
    return addMwCallSiteToError(async (ctx, next) => {
        ...
        if (!isIsomorphic) {
            const { received: additionalBnodes, expected: missingBnodes } = getBnodeDiff([...receivedQuads], [...expectedQuads]);
            report.blanknodes = {
...
const deSkolimizeNode = (node) => {
    if ((node.termType !== "NamedNode" && node.termType !== "DefaultGraph") || node.value === "")
        return node;
    let url;
    try {
...
const deSkolimizeQuads = (quads) => {
    return quads.map((quad) => {
        return dataFactory().quad(quad.subject.termType === "Quad" ? deSkolimizeQuads([quad.subject])[0] : deSkolimizeNode(quad.subject), quad.predicate, quad.object.termType === "Quad" ? deSkolimizeQuads([quad.object])[0] : deSkolimizeNode(quad.object), deSkolimizeNode(quad.graph));
    });
};
export const isIsomorphicReport = (value) => {
    return (value &&
        typeof value === "object" &&
...
const getReport = (report) => {
    ...
    if (report.quads.missing && report.quads.missing.length > 0) {
        msg.push(chalk.underline.grey("Missing Quads (that don't contain Blank Nodes):"), quadsToString(report.quads.missing));
    }
    if (report.blanknodes.missing && Object.entries(report.blanknodes.missing).length > 0) {
        msg.push(chalk.underline.grey("Missing BlankNode Patterns:"), Object.entries(report.blanknodes.missing)
            .map(([_, quads]) => quadsToString(quads))
            .join("\n"));
    }
    if (report.quads.additional && report.quads.additional.length > 0) {
        msg.push(chalk.underline.grey("Additional Quads (that don't contain Blank Nodes):"), quadsToString(report.quads.additional));
    }
    if (report.blanknodes.additional && Object.entries(report.blanknodes.additional).length > 0) {
        msg.push(chalk.underline.grey("Additional BlankNode Patterns:"), Object.entries(report.blanknodes.additional)
            .map(([_, quads]) => quadsToString(quads))
            .join("\n"));
    }
    return msg.join("\n").trim();
 ...
 */
function getNonBlankDiff(a1, a2) {
    const quads = a1.filter((quad) => everyTerms(quad, (term) => term.termType !== "BlankNode") && a2.every((q2) => !q2.equals(quad)));
 ...
 */
function getDiff(hash1, hash2) {
    const diffed = [];
    const hash1Values = Object.values(hash1);
    const hash1ValuesCount = {};
    for (const val of hash1Values)
        hash1ValuesCount[val] = !hash1ValuesCount[val] ? 1 : hash1ValuesCount[val] + 1;
    const hash2Values = Object.values(hash2);
    const hash2ValuesCount = {};
    for (const val of hash2Values)
        hash2ValuesCount[val] = !hash2ValuesCount[val] ? 1 : hash2ValuesCount[val] + 1;
    /**
...
function unGroundHashes(graph) {
    return hashTerms(uniqGraph(getQuadsWithBlankNodes(graph)), getGraphBlankNodes(graph), {})[1];
}
export function getBnodeDiff(receivedQuads, expectedQuads) {
...
//# sourceMappingURL=compareGraphs.js.map