...
export default function logRecord(opts) {
    return addMwCallSiteToError(async function _logRecord(ctx, next) {
        ...
        if (opts?.destination) {
            if (path.extname(opts.destination).toLowerCase() === ".json") {
                ...
                throw new Error('Please provide a valid file name for destination with JSON extension (e.g. "logRecord.json", "./data/logRecord.json")');
            }
        }
        else {
            console.info(record);
