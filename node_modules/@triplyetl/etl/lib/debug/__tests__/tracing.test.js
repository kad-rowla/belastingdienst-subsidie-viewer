...
describe("Tracing", function () {
    describe("traceStart and traceEnd middlewares", function () {
        ...
        it("Should handle forgetting to set traceEnd", async function () {
            ...
            await expect(pipeline.run()).to.eventually.be.rejectedWith("Trace info already exist. Did you forget to add a traceEnd");
        });
    ...
    describe("Trace on Error", function () {
        const fooGood = Array.from({ length: 1000 }, () => ({ foo: "bar", bar: ["foo", "bar"] }));
        ...
        it("Should correctly track diff of any 'record' data added between the start of the record and the error", async function () {
            ...
            etl["createErrorTrace"] = true;
            etl.use(fromJson(fooData));
            etl.use(addValue({ key: "beforeError", content: str("someValueAdded") }), when("fooo", triple(a, a, literal("foo", xsd.string))));
            try {
                await etl.run();
            }
            catch (_e) {
        ...
        it("Should correctly track diff of any 'Quad' data added between the start of the record and the error", async function () {
            ...
            etl["createErrorTrace"] = true;
            etl.use(fromJson(fooData));
            etl.use(triple(a, a, literal(str("addLiteral"), xsd.string)), when("fooo", triple(a, a, literal("foo", xsd.string))));
            try {
                await etl.run();
            }
            catch (_e) {
        ...
        it("Should trace correctly on error that occurs inside a forEach", async function () {
            ...
            etl["createErrorTrace"] = true;
            etl.use(fromJson(fooData));
            etl.use(when("fooo", addValue({ key: "newKey", content: "someInfo" })), when("fooo", forEach("bar", triple(a, a, literals(str("shoud throw an error"), xsd.string)))));
            try {
                await etl.run();
            }
            catch (_e) {
