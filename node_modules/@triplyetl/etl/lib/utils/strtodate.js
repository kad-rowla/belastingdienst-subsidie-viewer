...
export default function strToDate(text, now) {
    text = text
        .trim()
        .replace(/\s{2,}/g, " ")
        .replace(/[\t\r\n]/g, "")
    ...
    if (text === "")
        return new Date();
    const _now = now === undefined ? new Date() : new Date(now.getTime());
    ...
    const lastNext = (type, range, modifier) => {
        let diff;
    ...
    const process = (val) => {
        ...
        const type = splt[0];
        const range = splt[1].substring(0, 3);
        ...
        if (Object.prototype.hasOwnProperty.call(ranges, range) && splt[1].match(/^mon(day|\.)?$/i) == null) {
            const fnSet = `set${ranges[range]}`;
            const fnGet = `get${ranges[range]}`;
    ...
    const humanReadabletimes = new RegExp(/^[0-9]* +(years?|y|months?|weeks?|w|days?|d|hours?|h|minutes?|m|seconds?|s)$/i);
    if (humanReadabletimes.test(text)) {
        ...
        if (hrTime.startsWith("month"))
            return fns.month(_now, +amount);
        const time = hrTime[0];
        return fns[time](_now, +amount);
    }
    const times = "(years?|months?|weeks?|days?|hours?|minutes?|mins?|seconds?|sec" +
        "|sunday|sun\\.?|monday|mon\\.?|tuesday|tue\\.?|wednesday|wed\\.?" +
        "|thursday|thu\\.?|friday|fri\\.?|saturday|sat\\.?)";
    const regex = "([+-]?\\d+\\s" + times + "|" + "(last|next)\\s" + times + ")(\\sago)?";
    match = text.match(new RegExp(regex, "gi"));
    if (match === null)
        throw new Error(`Could not parse '${text}' as a date.`);
...
//# sourceMappingURL=strtodate.js.map