...
const headOption = new Option(
  "--head [number]",
  "Only process the first <n> number of records. Defaults to <n> 10 when not specified.",
);
...
program
  ...
  .option(
    "--verbose",
    "make the ETL print out messages, use multiple times to increase the verbosity",
    (_, previousVal) => previousVal + 1,
    0,
  )
  ...
  .option(
    "--account <accountname>",
    "TriplyDB account name (default: $TRIPLYDB_ACCOUNT, or the account that owns the API token)",
    defaultTriplyDBAccount,
  )
  .option(
    "--api <url>",
    "URL of the triply API. (default: $TRIPLYDB_API, or the API where the token is registered)",
    defaultTriplyDBApi,
  )
  .option(
    "--trace <tracefile>",
    "Debug an ETL pipeline using a trace file. To create a tracefile, use the mw.debug.traceStart() and mw.debug.traceEnd() middlewares",
  )
  .option(
    "--trace-error <tracefile>",
    "Debug an Error throw in an ETL pipeline using a trace file. These files are created automatically when an Error is produced during an ETL run",
  )
  .option("--log-dir <dir>", "Store log files in this directory")
  .option(
    "--timeout <time>",
    "Timeout ETL after set duration with human readable string (e.g. '15 seconds', '30 mins', '1 hour 20 minutes')",
  )
...
process.on("uncaughtException", function (err) {
  ...
  render(
    <CliContext.Provider
      value={{ useCmdInCliFeedback, dataDir: options.dataDir, verbosity: 1, cmd: cmd, etlFile: relativeEtlFile }}
    >
      <Stacktrace error={err} verbosity={1} />
    </CliContext.Provider>,
  );
  process.exit(1);
});
...
if (customArgs.length !== 1) {
  if (customArgs.length === 0) {
    ...
    console.error(
      chalk.red(
        `You provided the following ETL files as argument: ${customArgs.join(", ")}. Please only provide one file.`,
      ),
    );
    console.error();
    program.outputHelp();
...
if (options.timeout) {
  try {
    ...
    console.error(
      chalk.red(
        `Invalid --timeout argument given. Expected a human readable duration (e.g. '15 seconds', '30 mins', '1 hour 20 minutes').\n${error.message}`,
      ),
    );
    program.outputHelp();
    process.exit(1);
...
if (etlFile.endsWith(".ts")) {
  console.error(
    chalk.red(
      `File ${customArgs[0]} references a typescript file. Please provide a javascript file as argument instead`,
    ),
  );
  process.exit(1);
}

if (etlFile.endsWith(".")) etlFile = `${etlFile}js`;
if (!etlFile.endsWith(".js")) etlFile = `${etlFile}.js`;
...
let debugTrace: SerializedTraceInfo | undefined;
if (options.trace) {
...
if (!initError && typeof getApp.default !== "function") {
  console.error(
    chalk.red(`Expected ${etlFile} to export a default function, e.g. \`export default function(){return new Etl()}\``),
  );
  process.exit(1);
}
if (options.plain) process.env["CI"] = "true";
...
const { waitUntilExit } = render(
  ...
  >
    <ErrorBoundary>
      <Main initError={initError} getEtl={getApp?.default} />
    </ErrorBoundary>
  </CliContext.Provider>,
);
waitUntilExit()
