...
import { fromJson, Iri, MiddlewareList } from "../generic/index.js";
...
import {
  ...
  forEach,
  _switch,
} from "../generic/middlewares/conditional/index.js";
...
import {
  ...
  padStart,
  padEnd,
} from "../ratt/middlewares/transformations/index.js";
...
const allTransformingMiddlewares: MiddlewareList = [
...
export default async function (): Promise<Etl> {
  ...
  app.use(allTransformingMiddlewares, async (ctx, next) => {
    assert(
      Object.keys(ctx.record).length === allTransformingMiddlewares.length,
      `Expected ${allTransformingMiddlewares.length} keys, but found ${Object.keys(ctx.record).length}`,
    );
    return next();
  });
  return app;
