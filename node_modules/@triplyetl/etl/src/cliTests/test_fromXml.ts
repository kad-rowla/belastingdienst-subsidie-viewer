...
import fromXml, { JsonXmlNode } from "../generic/middlewares/sources/fromXml.js";
...
export default async function (): Promise<Etl> {
  ...
  app.use(
    fromXml([app.sources.first, app.sources.second], {
   ...
   */
  app.useWhen(
    (ctx) => isEqual(ctx.record.$selector, "Result.Performer"),
    triple(iri(app.prefix.foaf, "PresentationName[0].$text"), rdf("type"), "PerformerCatalogue[0].$text"),
  );

  app.useWhen(
    (ctx) => isEqual(ctx.record.$selector, "Result.ResultInfo"),
    triple(iri(app.prefix.rdf, "RequestedPerformerLink[0].$text"), rdf("yields"), "ReturnedPerformerLink[0].$text"),
  );
  app.use((ctx, next) => {
