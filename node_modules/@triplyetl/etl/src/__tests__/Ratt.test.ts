...
import Etl, { MwRuntimeInfo } from "../generic/Etl.js";
import { mapValues } from "lodash-es";
import { Destination, fromJson, Middleware } from "../generic/index.js";
...
describe("Etl", function () {
  ...
  it("should stop at head, even if `next` isn't called", async function () {
    ...
    app.use(async (ctx) => {
      if ("d" in ctx.record) throw new Error("should've stopped at head");
    });
  ...
  it("should skip the first few records with fromRecordId", async function () {
    ...
    app.use(fromJson([{ a: "a" }, { b: "b" }, { c: "c" }, { d: "d" }]));
    app.use(async (ctx) => {
      if ("a" in ctx.record) throw new Error("should've skipped a");
      if ("b" in ctx.record) throw new Error("should've skipped b");
      nRecords++;
    });
    await app.run();
  ...
  it("should combine fromRecordId and head", async function () {
    ...
    app.use(async (ctx) => {
      if ("a" in ctx.record) throw new Error("should've skipped a");
      if ("b" in ctx.record) throw new Error("should've skipped b");
      if ("e" in ctx.record) throw new Error("should've stopped after d");
      nRecords++;
    });
    await app.run();
    expect(nRecords).to.equal(2);
  ...
  describe("Should track the correct current middleware index", function () {
    function throwWhenTouched(): Middleware {
      return async () => {
  ...
  it("Should resume correctly using a trace", async function () {
    ...
    function checkIndexMw(expectedIndex: Array<number>): Middleware {
  ...
  describe("After hooks", async function () {
    ...
    it("should run destination post-processing first", async function () {
      ...
      app.use((ctx) =>
        ctx.writeQuads(
          [quad(dataFactory.namedNode("a:a"), dataFactory.namedNode("b:b"), dataFactory.namedNode("c:c"))],
          fileDest,
        ),
      );
      app.after(async () => {
  ...
  describe("Timing logs", function () {
    function bigintToRoundedMs(val?: bigint) {
      if (val === undefined) throw new Error("Expected bigint, got undefined");
      return Math.round(Number(val) / 1_000_000);
    }
    // @ts-ignore utility that we can use for debugging tests
    function prettyRuntimes(runtimes: { [mwIndicesPosition: string]: MwRuntimeInfo }) {
      function bigintToPrettyMs(val?: bigint) {
    ...
    it("Should report foreach middleware correctly", async function () {
      ...
      app.use(
        forEach("someArray", async (_, next) => {
