...
import { toRdf, Etl, Middleware, Destination, Source } from "../../../index.js";
import { getStreamParser } from "@triplydb/data-factory";

function getTestReportLocation(fileName: string) {
  return `./src/__tests__/resources/validationFiles/report_shacl_${fileName}`;
}
function getTestReportDestination(fileName: string) {
...
describe("Copying to a file destination", function () {
  ...
  it("to a dynamic destination", async function () {
    const app = new Etl({ defaultGraph: "https://default.com/" });
    // @ts-expect-error
    const failedCopy = app.copySource(Source.file("./src/__tests__/resources/shapeFiles/places.shp"), (ctx) =>
      Destination.file(ctx.getString("hallo"))
    );
  ...
  describe("when an error occurs", async function () {
    ...
    it("with distinct destinations, only the `always` destination should be written to", async function () {
      ...
      await expect(app.run()).to.eventually.rejectedWith(
        /The instance data of record 1 does not conform to the model./
      );
