...
describe("Test Iri", () => {
  it("should implement https://git.triply.cc/triply/etl/-/issues/265#note_54174", async () => {
    ...
    const etl = new Etl();
    etl.use(
      fromJson({ "@priref": 666, type: 123456, name: "ds" }),
      triple(iri(person, "@priref"), a, sdo.Person),
      triple(iri(person, "@priref"), sdo.additionalType, o),
      triple(iri(person, "@priref"), sdo.additionalType, iri(aat, "type")),
      triple(iri(dataset, "name"), owl.imports, sdo.toIri()),
      async (ctx, next) => {
