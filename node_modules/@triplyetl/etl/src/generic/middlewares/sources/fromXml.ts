...
const parseAttrs: (string: string) => { [attr: string]: string } = saxophone.parseAttrs;
...
export type JsonXmlNode = { [key: string]: JsonXmlNode[] | string } & { $text?: string };

async function iterateOverXml(
  selectors: string[],

  onRecordClose: (record: Partial<Record>) => Promise<void>,
  inputStream: pumpify,
  trim: boolean = true,
) {
  type IteratorOutput =
    | [nodeType: "tagopen", tag: string, attributes: string, isSelfClosing: ""]
    | [nodeType: "tagclose", tag: string]
    | [nodeType: "text", text: string]
    | [nodeType: "cdata", data: string]
    // we ignore irrelevant information (details at https://www.npmjs.com/package/async-saxophone)
    | [nodeType: "comment" | "processinginstruction", data: string];
  ...
  const stack: JsonXmlNode[] = [];

  // Not catching errors as part of #5041, as its implementation would be too
  // complicated for this AsyncIterable
  for await (const data of makeAsyncXMLParser({ noEmptyText: true, alwaysTagClose: true })(
    inputStream,
  ) as AsyncIterable<IteratorOutput>) {
    const [nodeType, tagName, attributes] = data;
    const currentNode = stack[stack.length - 1];
    if (nodeType === "tagopen") {
      currentPath.push(tagName);
      if (!selectors.some((selectorPath) => currentPath.join(".").startsWith(selectorPath))) continue;
      const node: JsonXmlNode = mapKeys(parseAttrs(attributes), (_, key) => `@${key}`);

      if (currentNode) {
        // add the node as a child of its parent.
        if (!currentNode[tagName]) currentNode[tagName] = [];
        (currentNode[tagName] as JsonXmlNode[]).push(node);
      }
      stack.push(node);
 ...
 */
export function validatePaths(paths: Array<string>) {
  if (!paths.length) return;
  let checkingIndex = 0;
  let keepChecking = true;
  while (keepChecking) {
    for (const [i, path] of paths.entries()) {
      if (checkingIndex === i) continue;
      if (paths[checkingIndex].startsWith(path)) {
        throw new Error(
          `A pattern starts with another pattern. Conflict between:\n - ${path}\n - ${paths[checkingIndex]}`,
        );
      }
    }
    checkingIndex++;
    if (checkingIndex === paths.length) keepChecking = false;
  }
}
 ...
 */
export default function fromXml(
  oneOrMoreSources:
    | string
  ...
  return addMwCallSiteToError(
    async function _fromXml(ctx, next) {
      assertOneExtractorPerETL(ctx.app, "fromXml");
      const selectors = castArray(opts.selectors);
      validatePaths(selectors);
      let filenameMap: Map<string, string | undefined> = new Map();
      let nonQuerySources: SourceGetter<"records", FileSourceInfo>[] = castArray(oneOrMoreSources).filter(
        (src) => !isQuerySourceGetter(src),
      ) as any as SourceGetter<"records", FileSourceInfo>[];
      const querySources: QuerySourceGetter[] = castArray(oneOrMoreSources).filter((src) =>
        isQuerySourceGetter(src),
      ...
      if (typeof oneOrMoreSources === "string") {
        ...
        for (const query of querySources) {
          for await (const xml of new SparqlResultsXmlIterator(ctx.app, query)) {
            ...
            await iterateOverXml(
              ...
              (record) => {
                const $tagName = record.$selector === undefined ? undefined : record.$selector.split(".").pop();
                const enrichement: { [key: string]: any } = opts.skipEnrich
                  ? {}
                  : {
                      $recordId: ctx.recordId + 1,
                      $environment: Etl.environment,
        ...
        for (const source of sources) {
          ...
          await iterateOverXml(
            ...
            (record) => {
              const $tagName = record.$selector === undefined ? undefined : record.$selector.split(".").pop();
              const enrichement: { [key: string]: any } = {
                $recordId: ctx.recordId + 1,
                $environment: Etl.environment,
                $tagName,
              };
