import { Etl, Middleware, MiddlewareList } from "../../index.js";
import { addMwCallSiteToError } from "../../../utils/index.js";
import { Record } from "../../Record.js";
class OnlyOnce {
  public static instance: Map<string, Record[]> = new Map();
  private constructor() {}
}
/**
...
export default function loadRecords(fromSrc: Middleware | MiddlewareList, key: string): Middleware {
  return addMwCallSiteToError(
    async function _loadRecords(ctx, next) {
      if (ctx.hasKey(key)) {
        throw new Error(`key '${key}' already exists`);
      }
      let records: Record[] = [];
      if (OnlyOnce.instance.has(ctx.app.id)) {
        records = OnlyOnce.instance.get(ctx.app.id) as Record[];
      } else {
        const etl = new Etl();
