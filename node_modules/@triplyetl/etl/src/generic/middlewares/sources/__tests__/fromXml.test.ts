...
import { JsonXmlNode } from "../fromXml.js";
...
describe("Read xml documents", function () {
  it("Should correctly read a single XML document with Array<string> selectors", async function () {
    ...
    app.use(
      fromXml([Source.file("./src/__tests__/resources/xmlFiles/1.xml")], {
  ...
  it("Should correctly read CDATA", async function () {
    ...
    const app = new Etl({ defaultGraph: "https://default.com/" });
    app.use(
      fromXml(Source.file("./src/__tests__/resources/xmlFiles/cdata.xml"), {
        selectors: "employee",
        trim: false,
        skipEnrich: true,
      }),
  ...
  it("Should correctly read two XML documents", async function () {
    ...
    const records: JsonXmlNode[] = [];
    app.use(async (ctx, next) => {
      records.push(ctx.record as JsonXmlNode);
      await next();
    });
    await app.run();
  ...
  it("should fix issue https://issues.triply.cc/issues/5631", async () => {
    const app = new Etl();
    app.use(
      fromXml(Source.file("./src/__tests__/resources/xmlFiles/issue-redmine-5631.xml"), {
        selectors: "a",
        skipEnrich: true,
      }),
      async (ctx, next) => {
        ...
        keys.forEach((key) =>
          ctx.getArray(key, "any").forEach((entry: { $text: string }) => assert.equal(entry.$text, entry.$text.trim())),
        );
        return next();
      },
    );
  ...
  it("should parse a string containing XML", async () => {
    const app = new Etl();
    app.use(
      fromXml("<root><name>John Doe</name></root>", { selectors: "root", skipEnrich: true }),
      async (ctx, next) => {
  ...
  it("should skip trimming values by default", async () => {
    ...
    app.use(
      ...
      async (ctx, next) => {
        ...
        const keys = ["header", "header-after"];
        keys.forEach((key) =>
          ctx.getArray(key, "any").forEach((entry: { $text: string }) => {
            if (entry.$text !== "value inside") assert.notEqual(entry.$text, entry.$text.trim());
          }),
        );
        return next();
      },
  ...
  it("should apply a stylesheet (XSL)", async () => {
    ...
    etl.use(
      ...
      addIri({ content: 'sdo:birthDate[0]["@rdf:datatype"]', key: "dataType" }),
      pairs(
        iri("@rdf:about"),
        [a, sdo.Person],
        [sdo.givenName, "sdo:givenName[0].$text"],
        [sdo.familyName, "sdo:familyName[0].$text"],
        [sdo.birthDate, literal("sdo:birthDate[0].$text", "dataType")],
      ),
      async (ctx, next) => {
        ...
        assert.equal(
          ctx.store.getQuads({ subject, predicate: sdo.familyName, object: getFactory().literal("Doe") }).length,
          1,
        );
  ...
  it("should fail on XSLT with saved query", async () => {
    ...
    return expect(app.run()).to.eventually.be.rejectedWith(
      /^We currently do not allow saved TriplyDB queries to be processed with XSLT.$/,
    );
  });
