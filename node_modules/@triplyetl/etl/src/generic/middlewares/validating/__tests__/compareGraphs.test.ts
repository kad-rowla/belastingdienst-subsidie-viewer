import { Middleware, MiddlewareList, Etl, fromJson, when, Source, Iri, loadRdf } from "../../../index.js";
import chai, { assert } from "chai";
import chaiAsPromised from "chai-as-promised";
import { addSkolemIri, iri, pairs, str, triple } from "../../../../ratt/index.js";
import compareGraphs, { isIsomorphicReport, IsomorphicReport } from "../compareGraphs.js";
...
export const assertDoeFamily = (): Middleware => {
...
export const writeDoeFamily: MiddlewareList = [
  async (ctx, next) => {
...
const isomorphicCheck: MiddlewareList = [
...
describe("Testing isomorphic between graphs", () => {
  ...
  it("Should assert 2 graphs are not isomorphic", () => {
    ...
    app.use(triple(iri(prefixes.ex, "id"), sdo.alternateName, str("J.")), isomorphicCheck, async (ctx, _) => {
      expect(() => ctx.getAny("isomorphic")).to.not.throw;
      expect(isIsomorphicReport(ctx.getAny("isomorphic"))).to.be.true;
      const report = ctx.getAny("isomorphic") as IsomorphicReport;
  ...
  it("Should throw when 2 graphs are not isomorphic", async () => {
    const app = getApp();
    app.use(
      triple(iri(prefixes.ex, "id"), sdo.alternateName, str("J.")),
      when((ctx) => ctx.getString("firstName") === "John", compareGraphs(Jane, { terminate: false, throw: true })),
      when((ctx) => ctx.getString("firstName") === "Jane", compareGraphs(John, { terminate: false, throw: true })),
    );
  ...
  it("Should work with skolem iri's", async () => {
    ...
    app.use(
      addSkolemIri({ prefix, key: "bn" }),
      pairs(
        iri(prefix, str("1")),
        [a, sdo.Person],
        [sdo.givenName, str("John")],
        [sdo.familyName, str("Doe")],
        [sdo.spouse, "bn"],
      ),
      pairs("bn", [a, sdo.Person], [sdo.givenName, str("Jane")]),
      compareGraphs(Source.file("src/__tests__/resources/rdfFiles/john-doe-with-blanknodes.trig")),
    );
  ...
  describe("Blanknodes that share predicate and object", function () {
    ...
    it("Should report missing bnodes", async function () {
      const etl = new Etl();
      etl.use(
        loadRdf(Source.file("src/generic/middlewares/validating/__tests__/resources/Bnodes-1-a.trig")),
        compareGraphs(Source.file("src/generic/middlewares/validating/__tests__/resources/Bnodes-1-b.trig"), {
          terminate: false,
          key: "isomorphic",
        }),
      );
      etl.use((ctx, next) => {
        const report = ctx.getAny("isomorphic") as IsomorphicReport;
        if (!report.blanknodes.missing) throw new Error("Should be some missing blanknodes");
    ...
    it("Should report additional bnodes", async function () {
      const etl = new Etl();
      etl.use(
        loadRdf(Source.file("src/generic/middlewares/validating/__tests__/resources/Bnodes-2-a.trig")),
        compareGraphs(Source.file("src/generic/middlewares/validating/__tests__/resources/Bnodes-2-b.trig"), {
          terminate: false,
          key: "isomorphic",
        }),
      );
      etl.use((ctx, next) => {
        const report = ctx.getAny("isomorphic") as IsomorphicReport;
        if (!report.blanknodes.additional) throw new Error("Should be some additional blanknodes");
    ...
    it("Should report missing and additional bnodes, symetrical", async function () {
      const etl = new Etl();
      etl.use(
        loadRdf(Source.file("src/generic/middlewares/validating/__tests__/resources/Bnodes-3-a.trig")),
        compareGraphs(Source.file("src/generic/middlewares/validating/__tests__/resources/Bnodes-3-b.trig"), {
          terminate: false,
          key: "isomorphic",
        }),
      );
    ...
    it("Should report missing and additional bnodes, asymetrical", async function () {
      const etl = new Etl();
      etl.use(
        loadRdf(Source.file("src/generic/middlewares/validating/__tests__/resources/Bnodes-4-a.trig")),
        compareGraphs(Source.file("src/generic/middlewares/validating/__tests__/resources/Bnodes-4-b.trig"), {
          terminate: false,
          key: "isomorphic",
        }),
      );
