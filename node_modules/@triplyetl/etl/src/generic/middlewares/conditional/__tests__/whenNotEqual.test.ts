...
describe("Conditional.whenNotEqual ", () => {
  it("Should work on single values", () => {
    return getEtlForTestingNoExtractor([
      ...
      whenNotEqual("bar", "1", addValue({ content: true, key: "checkIsNotEqual#14" })), // non existing key: do not run mw

      whenNotEqual("foo", "not-bar", addValue({ content: true, key: "checkIsNotEqual#20" })),
      async (ctx, next) => {
        for (let i = 10; i <= 14; i++) assert.throws(() => ctx.getBoolean(`checkIsNotEqual#${i}`));
        for (let i = 20; i <= 20; i++) assert.doesNotThrow(() => ctx.getBoolean(`checkIsNotEqual#${i}`));
        return next();
      },
    ]).run();
  });
  ...
  it("Should work on tuple values", () => {
    return getEtlForTestingNoExtractor([
      ...
      whenNotEqual("bar", ["1", "BAR", "Bar"], addValue({ content: true, key: "checkIsNotEqual#15" })), // non existing key: do not run mw

      whenNotEqual("foo", [true, "not-bar", undefined], addValue({ content: true, key: "checkIsNotEqual#20" })),
      async (ctx, next) => {
        for (let i = 10; i <= 15; i++) assert.throws(() => ctx.getBoolean(`checkIsNotEqual#${i}`));
        for (let i = 20; i <= 20; i++) assert.doesNotThrow(() => ctx.getBoolean(`checkIsNotEqual#${i}`));
        return next();
      },
    ]).run();
  ...
  it("Should work on objects", () => {
    ...
    return getEtlForTestingNoExtractor([
      ...
      async (ctx, next) => {
        for (let i = 10; i <= 13; i++) assert.throws(() => ctx.getBoolean(`checkIsNotEqual#${i}`));
        for (let i = 20; i <= 20; i++) assert.doesNotThrow(() => ctx.getBoolean(`checkIsNotEqual#${i}`));
        return next();
      },
    ]).run();
