...
describe("Conditional.whenEqual ", () => {
  it("Should work on single values", () => {
    return getEtlForTestingNoExtractor([
      ...
      async (ctx, next) => {
        for (let i = 10; i <= 13; i++) assert.doesNotThrow(() => ctx.getBoolean(`checkIsEqual#${i}`));
        for (let i = 20; i <= 22; i++) assert.throws(() => ctx.getBoolean(`checkIsEqual#${i}`));
        return next();
      },
    ]).run();
  });
  ...
  it("Should work on tuple values", () => {
    return getEtlForTestingNoExtractor([
      ...
      whenEqual("NotAKey", ["NotAKey"], addValue({ content: true, key: "checkIsEqual#22" })),
      async (ctx, next) => {
        for (let i = 10; i <= 14; i++) assert.doesNotThrow(() => ctx.getBoolean(`checkIsEqual#${i}`));
        for (let i = 20; i <= 22; i++) assert.throws(() => ctx.getBoolean(`checkIsEqual#${i}`));
        return next();
      },
    ]).run();
  ...
  it("Should work on objects", () => {
    ...
    return getEtlForTestingNoExtractor([
      ...
      async (ctx, next) => {
        for (let i = 10; i <= 12; i++) assert.doesNotThrow(() => ctx.getBoolean(`checkIsEqual#${i}`));
        for (let i = 20; i <= 20; i++) assert.throws(() => ctx.getBoolean(`checkIsEqual#${i}`));
        return next();
      },
    ]).run();
  });
