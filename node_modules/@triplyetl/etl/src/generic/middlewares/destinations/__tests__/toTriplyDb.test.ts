...
import { DatasetMetadata } from "../../../locations/destinations/rdf.js";
dotenv.config();
const onlyWhenToken = process.env.TRIPLYDB_TOKEN ? describe : describe.skip;

function getDataset(): DatasetMetadata {
...
onlyWhenToken("toTriplyDB middleware", () => {
  it("should work with a dataset object", async () => {
    ...
    etl.use(
      fromJson({}),
      triple(iri(etl.standardPrefixes.id, str(dataset.name)), a, sdo.Thing),
      toTriplyDb({ dataset, opts: { existingMetadata: "merge" } }),
    );
    etl.after(async (_) => {
      await etl.triplyDb
        .getAccount()
