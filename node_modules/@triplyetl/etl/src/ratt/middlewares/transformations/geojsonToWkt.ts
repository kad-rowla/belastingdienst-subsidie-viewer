  ...
  return addMwCallSiteToError<R>(
    add({
      ...
      value: (context) => {
        ...
        if (typeof args.content === "string" || isJPath(content)) content = context.getAny(content);
        else if (isStatic(args.content)) content = JSON.parse(args.content.toString());
        return (
          (args.crs !== undefined ? geospatial.wktCrsPrefix(context.getIri(args.crs)) : "") +
