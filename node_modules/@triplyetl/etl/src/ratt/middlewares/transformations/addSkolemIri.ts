...
import { addMwCallSiteToError, PossibleIri, randomHash } from "../../../utils/index.js";
  ...
  return addMwCallSiteToError<R>(
    add({
      value: (context) => {
        let url: URL;
        if (args.prefix) {
          url = new URL(context.getIri(args.prefix).value);
        } else {
          url = context.app.baseUrl;
        }
        const path = "/.well-known/genid/";
        if (url.pathname.replace(/\/+$/, "") !== "" && url.pathname !== path) {
          throw new AssertionError({
            message: `Skolem IRI must start with '${path}', either provide it or just use a base url.`,
          });
        }

        if (url.protocol !== "http:" && url.protocol !== "https:") {
          throw new AssertionError({ message: `Skolem IRI must use the HTTP or HTTPS scheme, got '${url.protocol}'.` });
        }

        url.pathname = path + randomHash();
        return dataFactory().namedNode(url.toString());
      },
      key: args.key,
    }),
    { sourceFuncName: "_addSkolemIri" },
  );
}
