 ...
 * ```
 */
export default function <R = unknown>(args: { content: Key | StaticString | LanguageTag; key: string }): Middleware<R> {
  return addMwCallSiteToError<R>(
    async function (ctx, next) {
      ...
      set(ctx.record, args.key, coerceToLanguageTag(isKey(args.content) ? ctx.getAny(args.content) : args.content));
      return next();
    },
    { sourceFuncName: "_addTag" },
  );
}
