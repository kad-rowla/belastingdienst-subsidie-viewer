...
export default function <R = unknown>(args: {
  ...
  return addMwCallSiteToError<R>(
    add({
      value: (context) => {
        let iriString: string = "";
        //shortcut: if the prefix is an Iri, we can quickly return it:
        if (isIri(args.prefix)) return args.prefix.concat(context.getString(args.content));

        if (args.prefix !== undefined) iriString = context.getIri(args.prefix).value;
        iriString += context.getString(args.content);
        try {
          const url = new URL(iriString, context.app.baseUrl);
          return dataFactory().namedNode(url.toString());
        } catch (e) {
          throw new Error(`addIri reported: can not make value '${iriString}' into a valid URL (key: '${args.key}')`);
        }
      },
      key: args.key,
    }),
    { sourceFuncName: "_addIri" },
  );
}
