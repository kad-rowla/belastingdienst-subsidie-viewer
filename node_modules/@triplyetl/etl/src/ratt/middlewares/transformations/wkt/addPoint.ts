...
export default function <R = unknown>(args: {
  crs?: PossibleIri | Key;
  latitude: StaticString | Key;
  longitude: StaticString | Key;
  key: string;
}): Middleware<R> {
  return addMwCallSiteToError<R>(
    add({
      value: (context) => {
        if (args.crs == "crs") args.crs = context.getString(args.crs);
        const lexicalString = `${
          args.crs ? `<${args.crs}> ` : "<http://www.opengis.net/def/crs/OGC/1.3/CRS84> "
        }Point (${context.getString(args.longitude)} ${context.getString(args.latitude)})`;
