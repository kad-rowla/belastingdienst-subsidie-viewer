...
describe("literal", () => {
  ...
  it("should use the runtime type", async function () {
    ...
    let stringLiteral: Literal | undefined;
    let numberLiteral: Literal | undefined;
    let booleanLiteral: Literal | undefined;
    ...
    expect((stringLiteral! as Literal).datatype.equals(xsd.string)).true;

    expect(numberLiteral).to.not.be.undefined;
    expect(isLiteral(numberLiteral)).true;
    expect((numberLiteral! as Literal).datatype.equals(xsd.integer)).true;

    expect(booleanLiteral).to.not.be.undefined;
    expect(isLiteral(booleanLiteral)).true;
    expect((booleanLiteral! as Literal).datatype.equals(xsd.boolean)).true;
  });
  ...
  it("Should create a rdf:langString", async () => {
    ...
    app.use(
      ...
      triple(iri("S"), iri("en"), literal("O", str("en"))),
      triple(iri("S"), iri("fr"), literal(str("Bonjour"), str("fr"))),
      triple(iri("S"), iri("es"), literal(str("Ola"), language.es)),
      triple(iri("S"), iri("nl"), literal(str("Hallo"), language.nl)),
      async (ctx, next) => {
        expect(ctx.store.getQuads(null, null, null, null).length).to.equal(4);
        const languages: string[] = ["en", "fr", "es", "nl"];
        languages.forEach((lang) => {
          ...
          expect((ctx.store.getObjects(null, ctx.getIri(lang), null)[0] as Literal).language).to.equal(lang);
        });
        return next();
      },
    );
  ...
  it("Should create a xsd:String", async () => {
    const app = new Etl({});
    app.use(
      addValue({ key: "foo", content: "foo1" }),
      triple(sdo.Thing, sdo.name, "foo"),
      triple(sdo.Thing, sdo.name, str("foo2")),
      async (ctx, next) => {
    ...
    app.use([
      ...
      async (ctx, next) => {
        ...
        ctx.store.getObjects(subject, sdo.name, null).forEach((lit) => {
          ...
          expect((lit as Literal).datatype.equals(xsd.string)).true;
        });
        ...
        expect((kilometers[0] as Literal).datatype.equals(DataFactory.namedNode("http://ex.com/unit/km"))).true;
        expect((kilometers[0] as Literal).value).equal("10");
        ...
        expect((miles[0] as Literal).datatype.equals(DataFactory.namedNode("http://ex.com/unit/miles"))).true;
        expect((miles[0] as Literal).value).equal("6.25");
        ...
        gender.forEach((gender) => {
          ...
          expect(
            (gender as Literal).datatype.equals(
              DataFactory.namedNode("http://www.w3.org/1999/02/22-rdf-syntax-ns#langString"),
            ),
          ).true;
          expect((gender as Literal).language).oneOf(["en", "de", "nl", "fr", "es"]);
        });
        return next();
      },
    ]);
  ...
  it("should work with misc. datatypes", () => {
    ...
    etl.use(async (ctx, next) => {
      ...
      Object.entries(types).forEach(([k, v]) => {
        ...
        expect((O[0] as Literal).language).equal("");
        expect((O[0] as Literal).datatype.equals(v)).true;
      });
      return next();
    });
    return etl.run();
  ...
  it("Should have the same datatypes for triple coerced key and literal middleware given datatype", async () => {
    const etlA = new Etl();
    const etlB = new Etl();
    let resultsA: Quad[] | undefined;
    let resultsB: Quad[] | undefined;
    etlA.use(
      fromJson({ String: "s", Number: 1, Boolean: false }),
      triple(a, a, literal("Boolean", xsd.boolean)),
      triple(a, a, literal("Number", xsd.integer)),
      triple(a, a, literal("String", xsd.string)),
      async (ctx) => {
  ...
  it("Should throw on wrong literal datatype given with estimated coercion datatype", async () => {
    ...
    etl.use(
      fromJson({ String: "s", Number: 1, Boolean: false }),
      triple(a, a, literal("Boolean", xsd.integer)),
      triple(a, a, literal("Number", xsd.integer)),
      triple(a, a, literal("String", xsd.string)),
    );
