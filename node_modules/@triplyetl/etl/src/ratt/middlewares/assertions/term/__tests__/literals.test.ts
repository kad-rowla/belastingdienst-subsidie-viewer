...
import { Context, Etl, forEach, fromJson, Iri, Middleware, NextFn } from "../../../../../generic/index.js";
...
describe("literals", () => {
  it("should work with static strings", () => {
    return getEtlForTestingNoExtractor([
      ...
      triple(
        prefix.concat(langString),
        vocab.sdo.alternateName,
        literals([str("Nederland"), str("Holland")], new vocab.LanguageTag(langString)),
      ),
      mwTester(),
    ]).run();
  });
  it("should work with a key holding an array", () => {
    return getEtlForTestingNoExtractor([
      ...
      triple(
        prefix.concat(langString),
        vocab.sdo.alternateName,
        literals("countryNames", new vocab.LanguageTag(langString)),
      ),
      mwTester(),
    ]).run();
  });
  it("should work with keys", () => {
    return getEtlForTestingNoExtractor([
      ...
      triple(
        prefix.concat(langString),
        vocab.sdo.alternateName,
        literals(["countryName1", "countryName2"], new vocab.LanguageTag(langString)),
      ),
      mwTester(),
    ]).run();
  });
  ...
  it("should work with mixed key/static", () => {
    return getEtlForTestingNoExtractor([
      ...
      triple(
        prefix.concat(langString),
        vocab.sdo.alternateName,
        literals(["countryName", str("Holland")], new vocab.LanguageTag(langString)),
      ),
      mwTester(),
    ]).run();
  });
  it("should fix issue https://git.triply.cc/triply/etl/-/issues/274", () => {
    const etl = new Etl();
    etl.use(
      fromJson([{ Record: [{ keywords: { Keyword: ["a"] } }] }, { Record: [{ keywords: { Keyword: ["b"] } }] }]),
      addIri({ content: "$recordId", key: "S" }),
      forEach(
        "Record",
        triple("$parent.S", vocab.sdo.keywords, literals("keywords.Keyword", vocab.xsd.string)),
        async (ctx, next) => {
