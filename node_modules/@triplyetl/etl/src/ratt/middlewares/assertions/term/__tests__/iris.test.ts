...
import { fromJson, Iri, Middleware } from "../../../../../generic/index.js";
...
describe("iris TermGetter", () => {
  it("should fail on a prefix with empty content", () => {
    return expect(getEtlForTesting([triple(iris(prefix, []), P, O)]).run()).eventually.rejectedWith(
      "Can not create Iri's using a prefix function with no local values.",
    );
  });
  it("should fail on a prefix with non-existing key", () => {
    ...
    ).eventually.rejectedWith("The key 'non-existing-key' is missing, unable to get a Iri values.");
  });
