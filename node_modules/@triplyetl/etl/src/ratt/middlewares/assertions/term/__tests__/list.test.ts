...
describe("toList", () => {
  it("Create list from source", async function () {
    ...
    app.use(
      triple(
        iri(app.prefix.basket, "id"),
        app.prefix.basket.concat("contains"),
        list(app.prefix.basket, literals("contains", xsd.string)),
      ),
    );
  ...
  it("Create list from mixed RecordGetters", async function () {
    ...
    expect((await fs.readFile(testDestinationPath)).toString().trim()).to.equal(
      `<https://www.triply.cc/fruitbasket/f6606e7783ab678c09e6898e8e63c461> <http://www.w3.org/1999/02/22-rdf-syntax-ns#first> "Banana" .
...
<https://www.triply.cc/fruitbasket/basket> <https://www.triply.cc/fruitbasket/contains> <https://www.triply.cc/fruitbasket/f6606e7783ab678c09e6898e8e63c461> .`,
  ...
  it("Able to specify a baseIri for List", async function () {
    ...
    await app.run();
    expect(app["_runtimeStats"]["#Statements"]).to.equal(7);
    expect((await fs.readFile(testDestinationPath2)).toString().trim()).to.equal(
      `<https://www.triply.cc/fruitbasket/00728b79289565b0cc4a74641b3b2dc7> <http://www.w3.org/1999/02/22-rdf-syntax-ns#first> "Banana" .
