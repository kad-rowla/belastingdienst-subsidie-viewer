...
import { Etl, fromJson, Iri, Middleware } from "../../../../../generic/index.js";
...
describe("iri TermGetter", () => {
  ...
  it("should fail with strings that are invalid URL's", () => {
    return expect(
      getEtlForTesting([triple(iri(str("err")), iri(str("P")), iri(str("O"))), validate()]).run(),
    ).eventually.rejectedWith(/Invalid Iri/);
  });
