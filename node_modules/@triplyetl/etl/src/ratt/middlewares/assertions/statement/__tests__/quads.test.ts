...
describe("writing.quads", () => {
  it("should create 5 triples in 2 quads", () => {
    return getEtlForTesting([
      ...
      quads(
        [vocab.sdo.Brewery, vocab.rdf.type, vocab.sdo.FoodEstablishment, graph],
        [vocab.sdo.Brewery, vocab.sdo.name, literal(str("Naam"), vocab.languageTag("nl")), graph],
        [vocab.sdo.Brewery, vocab.sdo.name, "objectIri", graph],
        [vocab.sdo.Brewery, vocab.sdo.name, "naam", anotherGraph],
        [vocab.sdo.Brewery, vocab.sdo.name, str("Heineken Brouwerij"), anotherGraph],
        [vocab.sdo.Brewery, vocab.sdo.name, "naamPrimitive", anotherGraph],
      ),
      async (ctx, next) => {
