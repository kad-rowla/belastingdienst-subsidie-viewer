  ...
  it("Should work on arrays with numbers", () => {
    return getEtlForTesting([
      ...
      async (ctx, next) => {
        ...
        assert.equal(
          ctx.store.getQuads({ predicate: sdo.numChildren, object: DataFactory.literal(3, vocab.xsd.integer) }).length,
          1,
        );
        assert.equal(
          ctx.store.getQuads({ predicate: sdo.numAdults, object: DataFactory.literal(3, vocab.xsd.integer) }).length,
          1,
        );
        assert.equal(
          ctx.store.getQuads({ predicate: sdo.numChildren, object: DataFactory.literal(4.123, vocab.xsd.float) })
            .length,
          1,
        );
        assert.equal(
          ctx.store.getQuads({ predicate: sdo.numAdults, object: DataFactory.literal(4.123, vocab.xsd.float) }).length,
          1,
        );
        assert.equal(
          ctx.store.getQuads({
      ...
      async (ctx, next) => {
        ...
        assert.isTrue(
          ctx.store
            ...
            .object.equals(DataFactory.literal("true", vocab.xsd.boolean)),
          "keyBoolean failed",
        );
        assert.isTrue(
          ctx.store.getQuads({ predicate: prefix.concat("keyIri") })[0].object.equals(prefix.concat("456")),
          "keyIri failed",
        );
        assert.isTrue(
          ctx.store
        ...
        assert.isTrue(
          ctx.store
            ...
            .object.equals(DataFactory.literal(123, vocab.xsd.integer)),
          "int failed",
        );
        assert.isTrue(
          ctx.store
            .getQuads({ predicate: prefix.concat("float") })[0]
            .object.equals(DataFactory.literal(123.456, vocab.xsd.float)),
          "float failed",
        );
        assert.isTrue(
          ctx.store.getQuads({ predicate: prefix.concat("string") })[0].object.equals(DataFactory.literal("string")),
          "string failed",
        );
        assert.isTrue(
          ctx.store.getQuads({ predicate: prefix.concat("literal") })[0].object.equals(DataFactory.literal("literal")),
          "literal failed",
        );
        assert.isTrue(
          ctx.store
            .getQuads({ predicate: prefix.concat("date") })[0]
            .object.equals(DataFactory.literal(now.toISOString(), vocab.xsd.dateTime)),
          "date failed",
        );
        return next();
      },
