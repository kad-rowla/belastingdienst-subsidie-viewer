...
const addContent = (content: string, section: "body" | "intro" = "body") => {
...
const writeOutput = (filename: string) => {
...
export const skip = (argv: any, key: string): boolean =>
  argv[`skip-${key}`] !== undefined || argv[`skip-${key}`] === true;

const avatar = (avatarUrl: string | undefined, height: number = 64): string =>
...
const cmd: CommandModule = {
  ...
  handler: async (_argv) => {
    ...
    if (!skip(_argv, "intro")) addContent(`${info.branding.welcomeMessage.replace(/^\#+/, "").trim()}`);
    ...
    if (_argv.account) {
      ...
      app
        .getAccount(_argv.account as string)
        .then(async (account) => {
          await stats(account, consoleUrl, _argv, 2);
        })
        .then((_) => writeOutput(_argv.out as string))
        .catch((e) => {
      ...
      app
        ...
        .then(async (accounts) => {
          ...
          for await (const account of accounts) {
            ...
            if ((_argv.account && (_argv.account as string) === info.accountName) || !_argv.account) {
...
const stats = async (account: User | Org, consoleUrl: string, _argv: any, startLevel: number = 3) => {
  const info = await account.getInfo();
  ...
  if (info.type === "user") {
    ...
    if (info.orgs?.length) {
      ...
      for (const org of info.orgs) {
        addContent(
          `| [${org.accountName}](${consoleUrl}/${org.accountName}) | ${org.name} | ${avatar(org.avatarUrl, 32)} |`,
        );
      }
    }
  }
  ...
  if (!skip(_argv, "datasets")) {
    ...
    for await (const dataset of datasets) {
      if (iD === 0) addContent(`.H${startLevel + 1} Datasets`);
      iD++;
      const info = await dataset.getInfo();
      addContent(
        `.H${startLevel + 2} ${avatar(info.avatarUrl)}${info.displayName.trim() ? info.displayName : info.name}`,
      );
      ...
      if (info.topics.length) addContent(`| Topics | ${info.topics.map((t) => `- ${t}`).join("<br>")} |`);
      else addContent(`| Topics | - |`);
      if (info.exampleResources.length)
        addContent(`| Examples | ${info.exampleResources.map((ex) => `- ${ex}`).join("<br>")} |`);
      else addContent(`| Examples | - |`);
      ...
      if (!skip(_argv, "services")) {
        ...
        for await (const service of services) {
          if (iSe === 0) {
            addContent(`\n.H${startLevel + 3} Assets`);
            addContent(
              `| Name | Created | Loaded | # Statements | # Graphs | Status | In sync | \n| - | - | - | -: | -: | - | - |`,
            );
          }
          iSe++;
          const info = await service.getInfo();
          addContent(
            `| [${info.name}](${consoleUrl}/${accountName}/services) | ${info.createdAt} | ${info.loadedAt} | ${millify(
              info.numberOfLoadedStatements ?? 0,
            )} | ${millify(info.numberOfGraphs ?? 0)} | ${info.status} | ${info.outOfSync ?? false ? "✅" : "❌"}`,
          );
        }
      }
    }
  ...
  for await (const pinnedItem of pinnedItems) {
    ...
    addContent(
      `- [${info.displayName?.trim() ? info.displayName : info.name}](${consoleUrl}/${accountName}/${info.name})`,
    );
  }
  ...
  if (!skip(_argv, "queries")) {
    ...
    for await (const query of queries) {
      if (iQ === 0) addContent(`.H${startLevel + 1} Queries`);
      iQ++;
      ...
      if (info.variables?.length) {
        ...
        info.variables.forEach((v) =>
          addContent(
            `| ${v.name} | ${v.termType} | ${v.required ? "yes" : "no"} | ${
              v.defaultValue ? (v.termType === "Literal" ? `*${v.defaultValue}*` : `[${v.defaultValue}](iri)`) : "-"
            } |`,
          ),
        );
      }
  ...
  if (!skip(_argv, "stories")) {
    ...
    for await (const story of stories) {
      if (iS === 0) addContent(`.H${startLevel + 1} Stories`);
      iS++;
