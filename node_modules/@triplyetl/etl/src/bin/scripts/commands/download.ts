...
const cmd: CommandModule = {
  ...
  handler: async (_argv) => {
    ...
    ]).catch((e) => logError((e as Error).message));
  },
};

const logInfo = (info: string) => console.info(chalk.cyan(info));
const logError = (info: string) => console.error(chalk.red(info));
const logWarn = (info: string) => console.warn(chalk.yellow(info));

const getPath = (_argv: any, ...components: string[]) => {
  const p = path.join(_argv.out as unknown as string, ...components);
  if (!existsSync(p)) {
    try {
...
let accounts: Array<User | Org>;
const getAccounts = async (_argv: any, app: App) => {
  if (accounts && accounts.length) return accounts;
  accounts = [];
  if (_argv.account) {
    accounts.push(await app.getAccount(_argv.account as unknown as string));
  } else {
...
const writeJson = (dirname: string, object: any, filename?: string) =>
  writeFileSync(path.join(dirname, filename ? `${filename}.json` : "info.json"), JSON.stringify(object, null, 2));

const downloadInstance = (_argv: any, app: App) => {
  const outDir = getPath(_argv);
  app
    .getInfo()
    .then((info) => {
      writeJson(outDir, info);
      if (info.branding.banner) downloadFile(info.branding.banner, outDir);
      if (info.branding.logo) downloadFile(info.branding.logo, outDir);
      if (info.branding.logoLg) downloadFile(info.branding.logoLg, outDir);
    })
    .then((_) => logInfo(`saved instance info ${outDir}`))
    .catch((e) => logError((e as Error).message));
};

const downloadAccounts = async (_argv: any, app: App) => {
  (await getAccounts(_argv, app)).forEach(async (account) => {
    const info = await account.getInfo();
    if (info.type === "org" && skip(_argv, "orgs") && info.accountName !== _argv.account) return;
    if (info.type === "user" && skip(_argv, "users") && info.accountName !== _argv.account) return;
    const accountDir = getPath(_argv, "accounts", `${info.type}s`, info.accountName);
    writeJson(accountDir, info);
    if (info.avatarUrl) downloadFile(info.avatarUrl, path.join(accountDir), info.accountName);
    logInfo(`saved account ${accountDir}`);
  });
};

const downloadStories = async (_argv: any, app: App) => {
  if (!skip(_argv, "stories")) {
    (await getAccounts(_argv, app)).forEach(async (account) => {
      ...
      for await (const story of account.getStories()) {
        ...
        if (info.bannerUrl) downloadFile(info.bannerUrl, storyDir);
        let markdown = "";
...
<html lang="en">
  ...
  <body>
    <div class="container">${new Remarkable({ html: true }).render(markdown)}</div>
        ...
        if (!skip(_argv, "stories-html")) writeFileSync(paths.html, html, { encoding: "utf-8" });
...
const downloadQueries = async (_argv: any, app: App) => {
    ...
    (await getAccounts(_argv, app)).forEach(async (account) => {
      ...
      for await (const query of account.getQueries()) {
        ...
        if (info.numberOfVersions > 0) {
          let variables: { [variable: string]: string | undefined } | undefined;
          if (info.variables) {
            variables = {};
          ...
          await query
            ...
            .catch((e) => {
              logWarn(`query '${info.link}': ${(e as Error).message}`);
            });
        }
      }
    });
...
const downloadDatasets = async (_argv: any, app: App) => {
  if (skip(_argv, "datasets")) return;
  (await getAccounts(_argv, app)).forEach(async (account) => {
...
const downloadFile = (url: string, targetDir: string, basename?: string) => {
      ...
      if (!ext) {
        ...
        if (contentType) {
          ...
          rename(targetFile, to, (err) => {
            if (err) logWarn(`failed to rename '${targetFile}' => '${to}'`);
          });
        }
      }
    })
