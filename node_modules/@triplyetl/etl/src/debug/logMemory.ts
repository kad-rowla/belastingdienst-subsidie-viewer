  ...
  return async (ctx, next) => {
    if (!options?.logOnRecord || ctx.recordId % options.logOnRecord === 0) {
      ...
      if (options?.destination !== undefined) {
        if (options?.destination.length === 0) throw Error("Empty string is not a valid argument for 'directory'");
        const memInfo = {
          callCount,
        ...
        ctx.app.info(
          `CallCount: ${++callCount} | RecordId: ${ctx.recordId} | Heap (MB) used: ${Math.round(
            hpUsed
          )} / total: ${Math.round(hpTtl)}`
        );
      }
    }
    return next();
