...
describe("validateIri", function () {
  ...
  it("Should throw, ratt issue #82 - invalid Iri", function () {
    expect(() => {
      validateIri(dataFactory.namedNode("•http://ex.com"));
    }).to.throw();
  });
});

describe("convertMwListToMiddleware", function () {
  const getWhenMwList = (trueOrFalse: boolean): MiddlewareList => [
...
describe("One extractor per ETL", () => {
  it("should fail on 2 extractors of the same type", async () => {
    ...
    await expect(app.run()).to.eventually.be.rejectedWith(
      "Extractor 'fromJson' can not be used: a ETL allows 1 extractor which currently is 'fromJson'.",
    );
  });
  it("should fail on 2 extractors of a different type", async () => {
    ...
    await expect(app.run()).to.eventually.be.rejectedWith(
      "Extractor 'fromJson' can not be used: a ETL allows 1 extractor which currently is 'fromCsv'.",
    );
  });
  it("should not fail with loadRecords() mw", async () => {
    const app = new Etl();
    app.use(
      loadRecords(fromJson({ foo: "bar" }), "foo"),
      fromCsv(Source.file("./src/__tests__/resources/csvFiles/people.csv")),
    );
