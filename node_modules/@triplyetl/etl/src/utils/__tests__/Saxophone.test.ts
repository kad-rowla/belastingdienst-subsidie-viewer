...
import { AsyncXMLParserOptions, makeAsyncXMLParser } from "../Saxophone.js";
 ...
 */
const expectEvents = async (xml: string | string[], events?: any, options?: Partial<AsyncXMLParserOptions>) => {
...
describe("Async Saxophone tests", function () {
  it("should parse comments", async () =>
    expectEvents("<!-- this is a comment -->", [["comment", " this is a comment "]]));
  ...
  it("should not parse invalid comments", async () => {
    await expect(expectEvents("<!-- this is an -- invalid comment ->")).to.eventually.be.rejectedWith(
      "Unexpected -- inside comment"
    );
  });

  it("should parse CDATA sections", async () =>
    expectEvents("<![CDATA[this is a c&data s<>ction]]>", [["cdata", "this is a c&data s<>ction"]]));

  it("should parse CDATA sections between two chunks", async () =>
    expectEvents(["<", "!", "[", "C", "D", "A", "T", "A", "[", "contents]]>"], [["cdata", "contents"]]));

  it("should not parse invalid CDATA sections", async () => {
    await expect(expectEvents(["<![CDAthis is NOT a c&data s<>ction]]>"])).to.eventually.be.rejectedWith(
      "Unrecognized sequence: <!["
    );
  });

  it("should not parse unclosed CDATA sections", async () => {
    await expect(expectEvents(["<![CDATA[this is a c&data s<>ction]>"])).to.eventually.be.rejectedWith(
  ...
  it("should not parse unclosed processing instructions", async () => {
    await expect(expectEvents('<?xml version="1.0" encoding="UTF-8">')).to.eventually.be.rejectedWith(
  ...
  it("should allow selecting only open tags", async () =>
    expectEvents(
      allNodes,
      [
        ["tagopen", "title", "", ""],
  ...
  it("should allow selecting only closing tags", async () =>
    expectEvents(
      allNodes,
      [
        ["tagclose", "title"],
  ...
  it("should allow selecting only opening and closing tags and text", async () =>
    expectEvents(
      allNodes,
      [
        ["tagopen", "title", "", ""],
  ...
  it("should allow selecting only comments", async () =>
    expectEvents(allNodes, [["comment", " this is a comment "]], { include: "comment" }));

  it("should allow selecting only cdata", async () =>
    expectEvents(allNodes, [["cdata", "this is a c&data s<>ction"]], { include: "cdata" }));

  it("should allow selecting only processing instructions", async () =>
    expectEvents(allNodes, [["processinginstruction", 'xml version="1.0" encoding="UTF-8"']], {
      include: "processinginstruction",
    }));

  it("should allow outputting tagclose for self-closing tags", async () =>
    expectEvents(
      openAndClose,
      [
        ["tagopen", "full", "", ""],
  ...
  it("should not add a tagclose for self-closing tags if no closeTagOnSelfClose option", async () =>
    expectEvents(
      openAndClose,
      [
        ["tagopen", "full", "", ""],
  ...
  it("should include text that is only whitespace by default", async () =>
    expectEvents(
      `<full>something</full>
        <selfclose />`,
      [
  ...
  it("should allow filtering out whitespace text", async () =>
    expectEvents(
      `<full>something</full>
        <selfclose />`,
      [
  ...
  it("should err on unclosed CDATA", async () => {
    await expect(expectEvents("<cdata-section><![CDATA[this is a c&data s<>ction")).to.eventually.be.rejectedWith(
      /Unclosed CDATA section/
    );
  });
  ...
  it("should parse text at end of the XML", async () =>
    expectEvents("<test />Extra text", [
      ["tagopen", "test", "", "/"],
      ["text", "Extra text"],
    ]));
});
