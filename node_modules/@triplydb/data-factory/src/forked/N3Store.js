  ...
  get size() {
    ...
    for (const graphKey in graphs)
      for (const subjectKey in (subjects = graphs[graphKey].subjects))
        for (const predicateKey in (subject = subjects[subjectKey])) size += Object.keys(subject[predicateKey]).length;
    ...
    for (const graphId in graphs) {
      ...
      if ((content = graphs[graphId])) {
        ...
        if (subject) {
          ...
          // If only subject and possibly predicate are given, the subject index will be the fastest
          else count += this._countInIndex(content.subjects, subjectId, predicateId, objectId);
        } else if (predicate) {
          // If only predicate and possibly object are given, the predicate index will be the fastest
    ...
    for (graph in graphs) {
      ...
      if ((content = graphs[graph])) {
        ...
        if (predicateId) {
          ...
          else this._loopByKey1(content.subjects, predicateId, callback);
        } else if (objectId)
